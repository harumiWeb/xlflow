...
`)
	...
	if !cfg.Analyze.DetectRangeFindNothingCheck || !cfg.Analyze.DetectObjectUseBeforeSet ||
		...
		!cfg.Analyze.DetectNonShortCircuitObjectGuard || !cfg.Analyze.DetectPublicAPITypeSafety || !cfg.Analyze.DetectUntrustedDataFlow || !cfg.Analyze.DetectExcelCellAccessInLoops {
...
[analyze]
disabled_rules = ["VBA201", "vba205", "VBA206", "VBA212", "VBA213", "VBA214", "VBA215", "VBA216", "vba217", "VBA218", "VBA219", "VBA220", "VBA221", "VBA222", "VBA223", "vba224", "VBA225", "VBA201"]
`)
	if err := os.WriteFile(filepath.Join(dir, FileName), body, 0o644); err != nil {
		t.Fatal(err)
	}
	...
	if cfg.Analyze.DetectPublicAPITypeSafety {
		t.Fatal("expected VBA222/detect_public_api_type_safety to be disabled")
	}
	...
	if cfg.Analyze.DetectExcelCellAccessInLoops {
		t.Fatal("expected VBA225/detect_excel_cell_access_in_loops to be disabled")
	}
	if !cfg.Analyze.DetectObjectUseBeforeSet {
		t.Fatal("expected unrelated analyze rule to remain enabled")
	}
	if got := strings.Join(cfg.Analyze.DisabledRules, ","); got != "VBA201,VBA205,VBA206,VBA212,VBA213,VBA214,VBA215,VBA216,VBA217,VBA218,VBA219,VBA220,VBA221,VBA222,VBA223,VBA224,VBA225" {
		t.Fatalf("disabled analyze rules = %q, want VBA201,VBA205,VBA206,VBA212,VBA213,VBA214,VBA215,VBA216,VBA217,VBA218,VBA219,VBA220,VBA221,VBA222,VBA223,VBA224,VBA225", got)
	}
}
