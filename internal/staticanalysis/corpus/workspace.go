...
func MaterializeThirdPartyProject(corpusRoot string, project Project, opts MaterializeOptions) (workspace MaterializedWorkspace, err error) {
	...
	// Corpus snapshots intentionally include the opt-in module-state and
	// discarded-return rules so that real-world evidence remains observable
	// without changing production defaults. Profile-specific exclusions
	// still take precedence when a profile deliberately omits a rule.
