...
func TestCommittedDiagnosticReviews(t *testing.T) {
	...
	if len(reviews) != 3467 {
		t.Fatalf("committed reviews = %d, want 3467", len(reviews))
	}
...
func TestCommittedCorpusReviewMetrics(t *testing.T) {
	...
	if metrics.Reviewed != 3709 || metrics.TP != 2357 || metrics.FP != 1352 {
		t.Fatalf("committed review metrics = %#v, want Reviewed=3709 TP=2357 FP=1352", metrics)
	}
