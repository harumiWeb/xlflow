...
func TestCommittedDiagnosticReviews(t *testing.T) {
	...
	if len(reviews) != 3473 {
		t.Fatalf("committed reviews = %d, want 3473", len(reviews))
	}
...
func TestCommittedCorpusReviewMetrics(t *testing.T) {
	...
	if metrics.Reviewed != 3715 || metrics.TP != 2357 || metrics.FP != 1358 {
		t.Fatalf("committed review metrics = %#v, want Reviewed=3715 TP=2357 FP=1358", metrics)
	}
