...
func TestCommittedDiagnosticReviews(t *testing.T) {
	...
	if len(reviews) != 3978 {
		t.Fatalf("committed reviews = %d, want 3978", len(reviews))
	}
...
func TestCommittedCorpusReviewMetrics(t *testing.T) {
	...
	if metrics.Reviewed != 4238 || metrics.TP != 2723 || metrics.FP != 1515 {
		t.Fatalf("committed review metrics = %#v, want Reviewed=4238 TP=2723 FP=1515", metrics)
	}
