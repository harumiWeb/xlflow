...
func TestCommittedDiagnosticReviews(t *testing.T) {
	...
	if len(reviews) != 3414 {
		t.Fatalf("committed reviews = %d, want 3414", len(reviews))
	}
...
func TestCommittedCorpusReviewMetrics(t *testing.T) {
	...
	if metrics.Reviewed != 3655 || metrics.TP != 2357 || metrics.FP != 1298 {
		t.Fatalf("committed review metrics = %#v, want Reviewed=3655 TP=2357 FP=1298", metrics)
	}
