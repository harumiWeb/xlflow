...
func TestCommittedDiagnosticReviews(t *testing.T) {
	...
	if len(reviews) != 2296 {
		t.Fatalf("committed reviews = %d, want 2296", len(reviews))
	}
...
func TestCommittedCorpusReviewMetrics(t *testing.T) {
	...
	if metrics.Reviewed != 2535 || metrics.TP != 1327 || metrics.FP != 1208 {
		t.Fatalf("committed review metrics = %#v, want Reviewed=2535 TP=1327 FP=1208", metrics)
	}
