...
func TestCommittedDiagnosticReviews(t *testing.T) {
	...
	if len(reviews) != 2645 {
		t.Fatalf("committed reviews = %d, want 2645", len(reviews))
	}
...
func TestCommittedCorpusReviewMetrics(t *testing.T) {
	...
	if metrics.Reviewed != 2884 || metrics.TP != 1676 || metrics.FP != 1208 {
		t.Fatalf("committed review metrics = %#v, want Reviewed=2884 TP=1676 FP=1208", metrics)
	}
