...
func TestCommittedDiagnosticReviews(t *testing.T) {
	...
	if len(reviews) != 1949 {
		t.Fatalf("committed reviews = %d, want 1949", len(reviews))
	}
...
func TestCommittedCorpusReviewMetrics(t *testing.T) {
	...
	if metrics.Reviewed != 2182 || metrics.TP != 1020 || metrics.FP != 1162 {
		t.Fatalf("committed review metrics = %#v, want Reviewed=2182 TP=1020 FP=1162", metrics)
	}
