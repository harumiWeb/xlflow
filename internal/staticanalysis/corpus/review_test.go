...
func TestCommittedDiagnosticReviews(t *testing.T) {
	...
	if len(reviews) != 3500 {
		t.Fatalf("committed reviews = %d, want 3500", len(reviews))
	}
...
func TestCommittedCorpusReviewMetrics(t *testing.T) {
	...
	if metrics.Reviewed != 3742 || metrics.TP != 2370 || metrics.FP != 1372 {
		t.Fatalf("committed review metrics = %#v, want Reviewed=3742 TP=2370 FP=1372", metrics)
	}
