...
func TestCommittedDiagnosticReviews(t *testing.T) {
	...
	if len(reviews) != 2725 {
		t.Fatalf("committed reviews = %d, want 2725", len(reviews))
	}
...
func TestCommittedCorpusReviewMetrics(t *testing.T) {
	...
	if metrics.Reviewed != 2964 || metrics.TP != 1755 || metrics.FP != 1209 {
		t.Fatalf("committed review metrics = %#v, want Reviewed=2964 TP=1755 FP=1209", metrics)
	}
