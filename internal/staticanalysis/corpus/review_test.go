...
func TestCommittedDiagnosticReviews(t *testing.T) {
	...
	if len(reviews) != 2779 {
		t.Fatalf("committed reviews = %d, want 2779", len(reviews))
	}
...
func TestCommittedCorpusReviewMetrics(t *testing.T) {
	...
	if metrics.Reviewed != 3018 || metrics.TP != 1809 || metrics.FP != 1209 {
		t.Fatalf("committed review metrics = %#v, want Reviewed=3018 TP=1809 FP=1209", metrics)
	}
