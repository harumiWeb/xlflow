...
func TestCommittedDiagnosticReviews(t *testing.T) {
	...
	if len(reviews) != 3654 {
		t.Fatalf("committed reviews = %d, want 3654", len(reviews))
	}
...
func TestCommittedCorpusReviewMetrics(t *testing.T) {
	...
	if metrics.Reviewed != 3892 || metrics.TP != 2464 || metrics.FP != 1428 {
		t.Fatalf("committed review metrics = %#v, want Reviewed=3892 TP=2464 FP=1428", metrics)
	}
