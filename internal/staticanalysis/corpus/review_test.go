...
func TestCommittedDiagnosticReviews(t *testing.T) {
	...
	if len(reviews) != 2905 {
		t.Fatalf("committed reviews = %d, want 2905", len(reviews))
	}
...
func TestCommittedCorpusReviewMetrics(t *testing.T) {
	...
	if metrics.Reviewed != 3144 || metrics.TP != 1929 || metrics.FP != 1215 {
		t.Fatalf("committed review metrics = %#v, want Reviewed=3144 TP=1929 FP=1215", metrics)
	}
