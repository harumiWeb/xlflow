...
func TestCommittedDiagnosticReviews(t *testing.T) {
	...
	if len(reviews) != 2825 {
		t.Fatalf("committed reviews = %d, want 2825", len(reviews))
	}
...
func TestCommittedCorpusReviewMetrics(t *testing.T) {
	...
	if metrics.Reviewed != 3064 || metrics.TP != 1849 || metrics.FP != 1215 {
		t.Fatalf("committed review metrics = %#v, want Reviewed=3064 TP=1849 FP=1215", metrics)
	}
