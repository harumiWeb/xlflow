...
func TestCommittedDiagnosticReviews(t *testing.T) {
	...
	if len(reviews) != 3458 {
		t.Fatalf("committed reviews = %d, want 3458", len(reviews))
	}
...
func TestCommittedCorpusReviewMetrics(t *testing.T) {
	...
	if metrics.Reviewed != 3700 || metrics.TP != 2357 || metrics.FP != 1343 {
		t.Fatalf("committed review metrics = %#v, want Reviewed=3700 TP=2357 FP=1343", metrics)
	}
