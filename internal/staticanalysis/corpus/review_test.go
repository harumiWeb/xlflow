...
func TestCommittedDiagnosticReviews(t *testing.T) {
	...
	if len(reviews) != 6512 {
		t.Fatalf("committed reviews = %d, want 6512", len(reviews))
	}
...
func TestCommittedCorpusReviewMetrics(t *testing.T) {
	...
	if metrics.Reviewed != 7393 || metrics.TP != 4742 || metrics.FP != 2651 {
		t.Fatalf("committed review metrics = %#v, want Reviewed=7393 TP=4742 FP=2651", metrics)
	}
