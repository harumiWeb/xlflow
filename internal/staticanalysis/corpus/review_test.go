...
func TestCommittedDiagnosticReviews(t *testing.T) {
	...
	if len(reviews) != 2745 {
		t.Fatalf("committed reviews = %d, want 2745", len(reviews))
	}
...
func TestCommittedCorpusReviewMetrics(t *testing.T) {
	...
	if metrics.Reviewed != 2984 || metrics.TP != 1775 || metrics.FP != 1209 {
		t.Fatalf("committed review metrics = %#v, want Reviewed=2984 TP=1775 FP=1209", metrics)
	}
