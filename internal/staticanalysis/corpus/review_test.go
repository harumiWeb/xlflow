...
func TestCommittedDiagnosticReviews(t *testing.T) {
	...
	if len(reviews) != 2431 {
		t.Fatalf("committed reviews = %d, want 2431", len(reviews))
	}
...
func TestCommittedCorpusReviewMetrics(t *testing.T) {
	...
	if metrics.Reviewed != 2670 || metrics.TP != 1462 || metrics.FP != 1208 {
		t.Fatalf("committed review metrics = %#v, want Reviewed=2670 TP=1462 FP=1208", metrics)
	}
