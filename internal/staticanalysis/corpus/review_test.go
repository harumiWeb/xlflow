...
func TestCommittedDiagnosticReviews(t *testing.T) {
	...
	if len(reviews) != 3648 {
		t.Fatalf("committed reviews = %d, want 3648", len(reviews))
	}
...
func TestCommittedCorpusReviewMetrics(t *testing.T) {
	...
	if metrics.Reviewed != 3890 || metrics.TP != 2476 || metrics.FP != 1414 {
		t.Fatalf("committed review metrics = %#v, want Reviewed=3890 TP=2476 FP=1414", metrics)
	}
