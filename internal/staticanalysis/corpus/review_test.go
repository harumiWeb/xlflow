...
func TestCommittedDiagnosticReviews(t *testing.T) {
	...
	if len(reviews) != 2565 {
		t.Fatalf("committed reviews = %d, want 2565", len(reviews))
	}
...
func TestCommittedCorpusReviewMetrics(t *testing.T) {
	...
	if metrics.Reviewed != 2804 || metrics.TP != 1596 || metrics.FP != 1208 {
		t.Fatalf("committed review metrics = %#v, want Reviewed=2804 TP=1596 FP=1208", metrics)
	}
