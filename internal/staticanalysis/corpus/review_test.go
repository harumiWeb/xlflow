...
func TestCommittedDiagnosticReviews(t *testing.T) {
	...
	if len(reviews) != 3476 {
		t.Fatalf("committed reviews = %d, want 3476", len(reviews))
	}
...
func TestCommittedCorpusReviewMetrics(t *testing.T) {
	...
	if metrics.Reviewed != 3718 || metrics.TP != 2357 || metrics.FP != 1361 {
		t.Fatalf("committed review metrics = %#v, want Reviewed=3718 TP=2357 FP=1361", metrics)
	}
