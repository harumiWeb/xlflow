...
func TestCommittedDiagnosticReviews(t *testing.T) {
	...
	if len(reviews) != 3966 {
		t.Fatalf("committed reviews = %d, want 3966", len(reviews))
	}
...
func TestCommittedCorpusReviewMetrics(t *testing.T) {
	...
	if metrics.Reviewed != 4226 || metrics.TP != 2713 || metrics.FP != 1513 {
		t.Fatalf("committed review metrics = %#v, want Reviewed=4226 TP=2713 FP=1513", metrics)
	}
