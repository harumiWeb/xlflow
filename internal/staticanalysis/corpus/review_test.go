...
func TestCommittedDiagnosticReviews(t *testing.T) {
	...
	if len(reviews) != 3503 {
		t.Fatalf("committed reviews = %d, want 3503", len(reviews))
	}
...
func TestCommittedCorpusReviewMetrics(t *testing.T) {
	...
	if metrics.Reviewed != 3745 || metrics.TP != 2372 || metrics.FP != 1373 {
		t.Fatalf("committed review metrics = %#v, want Reviewed=3745 TP=2372 FP=1373", metrics)
	}
