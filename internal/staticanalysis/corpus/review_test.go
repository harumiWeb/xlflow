...
func TestCommittedDiagnosticReviews(t *testing.T) {
	...
	if len(reviews) != 1431 {
		t.Fatalf("committed reviews = %d, want 1431", len(reviews))
	}
...
func TestCommittedCorpusReviewMetrics(t *testing.T) {
	...
	if metrics.Reviewed != 1558 || metrics.TP != 734 || metrics.FP != 824 {
		t.Fatalf("committed review metrics = %#v, want Reviewed=1558 TP=734 FP=824", metrics)
	}
