...
func TestCommittedDiagnosticReviews(t *testing.T) {
	...
	if len(reviews) != 9150 {
		t.Fatalf("committed reviews = %d, want 9150", len(reviews))
	}
...
func TestCommittedCorpusReviewMetrics(t *testing.T) {
	...
	if metrics.Reviewed != 10254 || metrics.TP != 7371 || metrics.FP != 2883 || metrics.Allowed != 90 || metrics.Unreviewed != 0 {
		t.Fatalf("committed review metrics = %#v, want Reviewed=10254 TP=7371 FP=2883 Allowed=90 Unreviewed=0", metrics)
	}
