...
func TestCommittedDiagnosticReviews(t *testing.T) {
	...
	if len(reviews) != 3835 {
		t.Fatalf("committed reviews = %d, want 3835", len(reviews))
	}
...
func TestCommittedCorpusReviewMetrics(t *testing.T) {
	...
	if metrics.Reviewed != 4087 || metrics.TP != 2590 || metrics.FP != 1497 {
		t.Fatalf("committed review metrics = %#v, want Reviewed=4087 TP=2590 FP=1497", metrics)
	}
