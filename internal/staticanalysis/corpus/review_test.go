...
func TestCommittedDiagnosticReviews(t *testing.T) {
	...
	if len(reviews) != 3595 {
		t.Fatalf("committed reviews = %d, want 3595", len(reviews))
	}
...
func TestCommittedCorpusReviewMetrics(t *testing.T) {
	...
	if metrics.Reviewed != 3837 || metrics.TP != 2451 || metrics.FP != 1386 {
		t.Fatalf("committed review metrics = %#v, want Reviewed=3837 TP=2451 FP=1386", metrics)
	}
