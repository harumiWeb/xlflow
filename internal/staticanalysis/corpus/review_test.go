...
func TestCommittedDiagnosticReviews(t *testing.T) {
	...
	if len(reviews) != 3617 {
		t.Fatalf("committed reviews = %d, want 3617", len(reviews))
	}
...
func TestCommittedCorpusReviewMetrics(t *testing.T) {
	...
	if metrics.Reviewed != 3859 || metrics.TP != 2451 || metrics.FP != 1408 {
		t.Fatalf("committed review metrics = %#v, want Reviewed=3859 TP=2451 FP=1408", metrics)
	}
