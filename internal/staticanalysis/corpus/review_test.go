...
func TestCommittedDiagnosticReviews(t *testing.T) {
	...
	if len(reviews) != 2000 {
		t.Fatalf("committed reviews = %d, want 2000", len(reviews))
	}
...
func TestCommittedCorpusReviewMetrics(t *testing.T) {
	...
	if metrics.Reviewed != 2236 || metrics.TP != 1068 || metrics.FP != 1168 {
		t.Fatalf("committed review metrics = %#v, want Reviewed=2236 TP=1068 FP=1168", metrics)
	}
