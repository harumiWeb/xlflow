...
func TestCommittedDiagnosticReviews(t *testing.T) {
	...
	if len(reviews) != 3492 {
		t.Fatalf("committed reviews = %d, want 3492", len(reviews))
	}
...
func TestCommittedCorpusReviewMetrics(t *testing.T) {
	...
	if metrics.Reviewed != 3734 || metrics.TP != 2362 || metrics.FP != 1372 {
		t.Fatalf("committed review metrics = %#v, want Reviewed=3734 TP=2362 FP=1372", metrics)
	}
