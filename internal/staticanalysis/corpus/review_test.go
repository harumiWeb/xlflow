...
func TestCommittedDiagnosticReviews(t *testing.T) {
	...
	if len(reviews) != 2925 {
		t.Fatalf("committed reviews = %d, want 2925", len(reviews))
	}
...
func TestCommittedCorpusReviewMetrics(t *testing.T) {
	...
	if metrics.Reviewed != 3164 || metrics.TP != 1949 || metrics.FP != 1215 {
		t.Fatalf("committed review metrics = %#v, want Reviewed=3164 TP=1949 FP=1215", metrics)
	}
