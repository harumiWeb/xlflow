...
func TestCommittedDiagnosticReviews(t *testing.T) {
	...
	if len(reviews) != 5620 {
		t.Fatalf("committed reviews = %d, want 5620", len(reviews))
	}
...
func TestCommittedCorpusReviewMetrics(t *testing.T) {
	...
	if metrics.Reviewed != 6187 || metrics.TP != 3542 || metrics.FP != 2645 {
		t.Fatalf("committed review metrics = %#v, want Reviewed=6187 TP=3542 FP=2645", metrics)
	}
