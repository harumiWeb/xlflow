...
func TestCommittedDiagnosticReviews(t *testing.T) {
	...
	if len(reviews) != 1938 {
		t.Fatalf("committed reviews = %d, want 1938", len(reviews))
	}
...
func TestCommittedCorpusReviewMetrics(t *testing.T) {
	...
	if metrics.Reviewed != 2166 || metrics.TP != 1005 || metrics.FP != 1161 {
		t.Fatalf("committed review metrics = %#v, want Reviewed=2166 TP=1005 FP=1161", metrics)
	}
