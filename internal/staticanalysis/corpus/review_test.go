...
func TestCommittedDiagnosticReviews(t *testing.T) {
	...
	if len(reviews) != 2665 {
		t.Fatalf("committed reviews = %d, want 2665", len(reviews))
	}
...
func TestCommittedCorpusReviewMetrics(t *testing.T) {
	...
	if metrics.Reviewed != 2904 || metrics.TP != 1696 || metrics.FP != 1208 {
		t.Fatalf("committed review metrics = %#v, want Reviewed=2904 TP=1696 FP=1208", metrics)
	}
