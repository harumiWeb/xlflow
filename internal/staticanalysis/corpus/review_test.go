...
func TestCommittedDiagnosticReviews(t *testing.T) {
	...
	if len(reviews) != 2236 {
		t.Fatalf("committed reviews = %d, want 2236", len(reviews))
	}
...
func TestCommittedCorpusReviewMetrics(t *testing.T) {
	...
	if metrics.Reviewed != 2475 || metrics.TP != 1267 || metrics.FP != 1208 {
		t.Fatalf("committed review metrics = %#v, want Reviewed=2475 TP=1267 FP=1208", metrics)
	}
