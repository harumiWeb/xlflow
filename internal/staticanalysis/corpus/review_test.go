...
func TestCommittedDiagnosticReviews(t *testing.T) {
	...
	if len(reviews) != 3472 {
		t.Fatalf("committed reviews = %d, want 3472", len(reviews))
	}
...
func TestCommittedCorpusReviewMetrics(t *testing.T) {
	...
	if metrics.Reviewed != 3714 || metrics.TP != 2357 || metrics.FP != 1357 {
		t.Fatalf("committed review metrics = %#v, want Reviewed=3714 TP=2357 FP=1357", metrics)
	}
