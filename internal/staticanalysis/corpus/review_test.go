...
func TestCommittedDiagnosticReviews(t *testing.T) {
	...
	if len(reviews) != 1436 {
		t.Fatalf("committed reviews = %d, want 1436", len(reviews))
	}
...
func TestCommittedCorpusReviewMetrics(t *testing.T) {
	...
	if metrics.Reviewed != 1565 || metrics.TP != 741 || metrics.FP != 824 {
		t.Fatalf("committed review metrics = %#v, want Reviewed=1565 TP=741 FP=824", metrics)
	}
