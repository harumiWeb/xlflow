...
func TestCommittedDiagnosticReviews(t *testing.T) {
	...
	if len(reviews) != 3801 {
		t.Fatalf("committed reviews = %d, want 3801", len(reviews))
	}
...
func TestCommittedCorpusReviewMetrics(t *testing.T) {
	...
	if metrics.Reviewed != 4052 || metrics.TP != 2555 || metrics.FP != 1497 {
		t.Fatalf("committed review metrics = %#v, want Reviewed=4052 TP=2555 FP=1497", metrics)
	}
