...
func TestCommittedDiagnosticReviews(t *testing.T) {
	...
	if len(reviews) != 2276 {
		t.Fatalf("committed reviews = %d, want 2276", len(reviews))
	}
...
func TestCommittedCorpusReviewMetrics(t *testing.T) {
	...
	if metrics.Reviewed != 2515 || metrics.TP != 1307 || metrics.FP != 1208 {
		t.Fatalf("committed review metrics = %#v, want Reviewed=2515 TP=1307 FP=1208", metrics)
	}
