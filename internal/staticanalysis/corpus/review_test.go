...
func TestCommittedDiagnosticReviews(t *testing.T) {
	...
	if len(reviews) != 3994 {
		t.Fatalf("committed reviews = %d, want 3994", len(reviews))
	}
...
func TestCommittedCorpusReviewMetrics(t *testing.T) {
	...
	if metrics.Reviewed != 4254 || metrics.TP != 2739 || metrics.FP != 1515 {
		t.Fatalf("committed review metrics = %#v, want Reviewed=4254 TP=2739 FP=1515", metrics)
	}
