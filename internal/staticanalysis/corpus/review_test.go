...
func TestCommittedDiagnosticReviews(t *testing.T) {
	...
	if len(reviews) != 2885 {
		t.Fatalf("committed reviews = %d, want 2885", len(reviews))
	}
...
func TestCommittedCorpusReviewMetrics(t *testing.T) {
	...
	if metrics.Reviewed != 3124 || metrics.TP != 1909 || metrics.FP != 1215 {
		t.Fatalf("committed review metrics = %#v, want Reviewed=3124 TP=1909 FP=1215", metrics)
	}
