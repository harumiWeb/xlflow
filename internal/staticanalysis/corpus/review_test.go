...
func TestCommittedDiagnosticReviews(t *testing.T) {
	...
	if len(reviews) != 2316 {
		t.Fatalf("committed reviews = %d, want 2316", len(reviews))
	}
...
func TestCommittedCorpusReviewMetrics(t *testing.T) {
	...
	if metrics.Reviewed != 2555 || metrics.TP != 1347 || metrics.FP != 1208 {
		t.Fatalf("committed review metrics = %#v, want Reviewed=2555 TP=1347 FP=1208", metrics)
	}
