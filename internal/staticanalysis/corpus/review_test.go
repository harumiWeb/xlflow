...
func TestCommittedDiagnosticReviews(t *testing.T) {
	...
	if len(reviews) != 1771 {
		t.Fatalf("committed reviews = %d, want 1771", len(reviews))
	}
...
func TestCommittedCorpusReviewMetrics(t *testing.T) {
	...
	if metrics.Reviewed != 1962 || metrics.TP != 801 || metrics.FP != 1161 {
		t.Fatalf("committed review metrics = %#v, want Reviewed=1962 TP=801 FP=1161", metrics)
	}
