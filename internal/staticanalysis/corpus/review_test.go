...
func TestCommittedDiagnosticReviews(t *testing.T) {
	...
	if len(reviews) != 3014 {
		t.Fatalf("committed reviews = %d, want 3014", len(reviews))
	}
...
func TestCommittedCorpusReviewMetrics(t *testing.T) {
	...
	if metrics.Reviewed != 3253 || metrics.TP != 2038 || metrics.FP != 1215 {
		t.Fatalf("committed review metrics = %#v, want Reviewed=3253 TP=2038 FP=1215", metrics)
	}
