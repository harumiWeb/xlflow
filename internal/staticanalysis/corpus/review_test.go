...
func TestCommittedDiagnosticReviews(t *testing.T) {
	...
	if len(reviews) != 3487 {
		t.Fatalf("committed reviews = %d, want 3487", len(reviews))
	}
...
func TestCommittedCorpusReviewMetrics(t *testing.T) {
	...
	if metrics.Reviewed != 3729 || metrics.TP != 2364 || metrics.FP != 1365 {
		t.Fatalf("committed review metrics = %#v, want Reviewed=3729 TP=2364 FP=1365", metrics)
	}
