...
func TestCommittedDiagnosticReviews(t *testing.T) {
	...
	if len(reviews) != 1968 {
		t.Fatalf("committed reviews = %d, want 1968", len(reviews))
	}
...
func TestCommittedCorpusReviewMetrics(t *testing.T) {
	...
	if metrics.Reviewed != 2203 || metrics.TP != 1039 || metrics.FP != 1164 {
		t.Fatalf("committed review metrics = %#v, want Reviewed=2203 TP=1039 FP=1164", metrics)
	}
