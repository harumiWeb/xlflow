...
func TestCommittedDiagnosticReviews(t *testing.T) {
	...
	if len(reviews) != 3961 {
		t.Fatalf("committed reviews = %d, want 3961", len(reviews))
	}
...
func TestCommittedCorpusReviewMetrics(t *testing.T) {
	...
	if metrics.Reviewed != 4221 || metrics.TP != 2708 || metrics.FP != 1513 {
		t.Fatalf("committed review metrics = %#v, want Reviewed=4221 TP=2708 FP=1513", metrics)
	}
