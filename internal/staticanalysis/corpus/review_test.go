...
func TestCommittedDiagnosticReviews(t *testing.T) {
	...
	if len(reviews) != 9153 {
		t.Fatalf("committed reviews = %d, want 9153", len(reviews))
	}
...
func TestCommittedCorpusReviewMetrics(t *testing.T) {
	...
	if metrics.Reviewed != 10257 || metrics.TP != 7374 || metrics.FP != 2883 || metrics.Allowed != 90 || metrics.Unreviewed != 0 {
		t.Fatalf("committed review metrics = %#v, want Reviewed=10257 TP=7374 FP=2883 Allowed=90 Unreviewed=0", metrics)
	}
	// This review pass expanded the ledger with confirmed VBA256, VBA257,
	// VBA258, VBA259, and VBA265-VBA269 findings, so the observed precision
	// remains close to the review floor.
