...
func TestCommittedDiagnosticReviews(t *testing.T) {
	...
	if len(reviews) != 3460 {
		t.Fatalf("committed reviews = %d, want 3460", len(reviews))
	}
...
func TestCommittedCorpusReviewMetrics(t *testing.T) {
	...
	if metrics.Reviewed != 3702 || metrics.TP != 2357 || metrics.FP != 1345 {
		t.Fatalf("committed review metrics = %#v, want Reviewed=3702 TP=2357 FP=1345", metrics)
	}
