...
func TestCommittedDiagnosticReviews(t *testing.T) {
	...
	if len(reviews) != 2625 {
		t.Fatalf("committed reviews = %d, want 2625", len(reviews))
	}
...
func TestCommittedCorpusReviewMetrics(t *testing.T) {
	...
	if metrics.Reviewed != 2864 || metrics.TP != 1656 || metrics.FP != 1208 {
		t.Fatalf("committed review metrics = %#v, want Reviewed=2864 TP=1656 FP=1208", metrics)
	}
