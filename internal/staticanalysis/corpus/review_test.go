...
func TestCommittedDiagnosticReviews(t *testing.T) {
	...
	if len(reviews) != 3501 {
		t.Fatalf("committed reviews = %d, want 3501", len(reviews))
	}
...
func TestCommittedCorpusReviewMetrics(t *testing.T) {
	...
	if metrics.Reviewed != 3743 || metrics.TP != 2370 || metrics.FP != 1373 {
		t.Fatalf("committed review metrics = %#v, want Reviewed=3743 TP=2370 FP=1373", metrics)
	}
