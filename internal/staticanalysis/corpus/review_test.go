...
func TestCommittedDiagnosticReviews(t *testing.T) {
	...
	if len(reviews) != 3713 {
		t.Fatalf("committed reviews = %d, want 3713", len(reviews))
	}
...
func TestCommittedCorpusReviewMetrics(t *testing.T) {
	...
	if metrics.Reviewed != 3962 || metrics.TP != 2533 || metrics.FP != 1429 {
		t.Fatalf("committed review metrics = %#v, want Reviewed=3962 TP=2533 FP=1429", metrics)
	}
