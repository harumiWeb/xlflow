...
func TestCommittedDiagnosticReviews(t *testing.T) {
	...
	if len(reviews) != 2545 {
		t.Fatalf("committed reviews = %d, want 2545", len(reviews))
	}
...
func TestCommittedCorpusReviewMetrics(t *testing.T) {
	...
	if metrics.Reviewed != 2784 || metrics.TP != 1576 || metrics.FP != 1208 {
		t.Fatalf("committed review metrics = %#v, want Reviewed=2784 TP=1576 FP=1208", metrics)
	}
