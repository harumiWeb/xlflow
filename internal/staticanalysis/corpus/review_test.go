...
func TestCommittedDiagnosticReviews(t *testing.T) {
	...
	if len(reviews) != 2765 {
		t.Fatalf("committed reviews = %d, want 2765", len(reviews))
	}
...
func TestCommittedCorpusReviewMetrics(t *testing.T) {
	...
	if metrics.Reviewed != 3004 || metrics.TP != 1795 || metrics.FP != 1209 {
		t.Fatalf("committed review metrics = %#v, want Reviewed=3004 TP=1795 FP=1209", metrics)
	}
