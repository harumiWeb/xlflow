...
func TestCommittedDiagnosticReviews(t *testing.T) {
	...
	if len(reviews) != 3996 {
		t.Fatalf("committed reviews = %d, want 3996", len(reviews))
	}
...
func TestCommittedCorpusReviewMetrics(t *testing.T) {
	...
	if metrics.Reviewed != 4256 || metrics.TP != 2741 || metrics.FP != 1515 {
		t.Fatalf("committed review metrics = %#v, want Reviewed=4256 TP=2741 FP=1515", metrics)
	}
