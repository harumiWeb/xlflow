...
func TestCommittedDiagnosticReviews(t *testing.T) {
	...
	if len(reviews) != 2256 {
		t.Fatalf("committed reviews = %d, want 2256", len(reviews))
	}
...
func TestCommittedCorpusReviewMetrics(t *testing.T) {
	...
	if metrics.Reviewed != 2495 || metrics.TP != 1287 || metrics.FP != 1208 {
		t.Fatalf("committed review metrics = %#v, want Reviewed=2495 TP=1287 FP=1208", metrics)
	}
