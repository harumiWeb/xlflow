...
func (r Runner) runWithOptions(commandName string, args map[string]string, opts commandRunOptions) (output.Envelope, int, error) {
	...
	case excelbridge.ModeAuto:
		execution = executeBridgeProvider(ctx, bridgeProviderForMode(r.RootDir, excelbridge.ModeDotNet), commandName, args)
	default:
...
func exitCodeForScriptResult(result ScriptResult) int {
	...
	case "process_enumeration_failed", "process_termination_failed", "process_cleanup_failed", "vba_component_remove_failed", "vba_component_import_name_mismatch":
		return output.ExitEnvironment
