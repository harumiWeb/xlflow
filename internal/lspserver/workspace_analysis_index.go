...
func (x *workspaceAnalysisIndex) buildInitial() {
	...
	if err == nil {
		for _, file := range files {
			...
			// project snapshot incomplete, but must not prevent the remaining
			// files from being indexed.  upsertDisk records that path in
			// incomplete; readiness errors are reserved for workspace discovery
			// failures so a later clean refresh can restore completeness.
...
func (x *workspaceAnalysisIndex) callGraphSnapshot() (callgraph.Snapshot, error) {
	...
	for _, ref := range x.all {
		...
		entry := x.effective[ref.path]
		file := workspaceDisplayPath(x.root, entry.path)
		resolverSymbols = append(resolverSymbols, procedureir.ResolverSymbol{Name: sym.Name, Type: sym.ReturnType, Module: sym.Module, ModuleKind: sym.ModuleKind, Kind: sym.Kind, Visibility: sym.Visibility, File: file, Line: sym.Range.Start.Line + 1, Parent: sym.Parent, IsArray: sym.IsArray})
		graphSymbols = append(graphSymbols, callgraph.Symbol{
			Name: sym.Name, Kind: sym.Kind, Module: sym.Module, ModuleKind: sym.ModuleKind, File: file,
