...
| ---: | -------------------------------------------------------------------------------------------------------------------------------------------------------------- |
|  `0` | Success                                                                                                                                                        |
|  `1` | Validation failure, such as an error-, unknown-, or empty-severity lint/analyze diagnostic (including source diagnostics from `check`), macro, or test failure |
...
>
> Lint/analyze results, or the source-diagnostic portion of `check`, containing only warning or information diagnostics return exit code `0`; inspect the diagnostics in the output to review them. Unknown or empty severities remain blocking and return exit code `1`.
