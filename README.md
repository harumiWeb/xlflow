...
`xlflow new/init --with-skill` installs the bundled `xlflow` AI agent skill during setup. The same skill can be installed later with `xlflow skill install`. Supported provider targets are `agents`, `codex`, `claude`, `cursor`, and `gemini`; when no provider is specified in an interactive terminal, xlflow shows a Bubble Tea selector. JSON and non-interactive runs must pass `--agent` or `--target`. For GitHub Copilot, use the shared `agents` target because Copilot reads repository instructions from `.agents`.
