...
- `requested_bridge` — requested bridge mode for this invocation.
- `selected_bridge` — always `"dotnet"` when the .NET bridge handles the command.
- `fallback` — always `false` when the .NET bridge handles the command.
- `legacy` — always `false` when the .NET bridge handles the command.
...
The Go resolver uses the supported command set to prefer `.NET` first in Windows `auto` mode. If `.NET` reports capability, runtime, protocol, or transport failures that make the command unavailable, the Go side may fall back to PowerShell only in `auto` mode. Explicit `--bridge dotnet` remains strict.

## Selection

Users can select the bridge through:
