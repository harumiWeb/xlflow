...
<!-- xlflow-rule-contract: {"id":"VBA260","family":"analyze","category":"maintainability","default_severity":"warning","scope":"procedure-local","realtime":true,"configuration_key":"detect_unused_parameters","inline_suppressible":true,"preflight_blocking":false} -->
<!-- xlflow-rule-contract: {"id":"VBA261","family":"analyze","category":"maintainability","default_severity":"warning","scope":"file-local","realtime":true,"configuration_key":"detect_unused_private_constants","inline_suppressible":true,"preflight_blocking":false} -->
<!-- xlflow-rule-contract: {"id":"VBA262","family":"analyze","category":"maintainability","default_severity":"information","scope":"file-local","realtime":true,"configuration_key":"detect_unused_udt_members","inline_suppressible":true,"preflight_blocking":false} -->
<!-- xlflow-rule-contract: {"id":"VBA263","family":"analyze","category":"reliability","default_severity":"warning","scope":"procedure-local","realtime":true,"configuration_key":"detect_never_assigned_variables","inline_suppressible":true,"preflight_blocking":false} -->
<!-- xlflow-rule-contract: {"id":"VBA264","family":"analyze","category":"reliability","default_severity":"warning","scope":"procedure-local","realtime":true,"configuration_key":"detect_unassigned_variable_usage","inline_suppressible":true,"preflight_blocking":false} -->

`VBA260`–`VBA264` extend the unused-declaration analysis family. All five
...
`VBA260` reports a parameter of a `Private` procedure that is never
...
`VBA261` reports a module-level `Private Const` that is never referenced.
...
`VBA260`, the scan is lexical and occurrences in comments or string literals
...
## VBA262 — unused user-defined type member

`VBA262` reports a member of a `Private Type` declaration that no resolvable
...
`VBA263` reports a procedure-local scalar variable that is read but never
assigned. `VBA264` reports a read that executes on a reachable control-flow
...
definition suppresses `VBA263` and joins the CFG's per-block assignment set
for `VBA264`, but the argument position itself is not counted as a proven
...
`VBA264` measures definite assignment over normal control-flow edges only.
...
event, `Implements`, and dynamic-entry exclusions for `VBA260`; constant
references in initializers and conditional compilation for `VBA261`; direct,
`With`-block, nested, and Variant-escape member resolution for `VBA262`;
never-assigned scalars, ByRef mutation, and straight-line/branching
definite-assignment reads for `VBA263`/`VBA264`. The rules do not depend on
