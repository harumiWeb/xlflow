...
`test list --json` is a source-only discovery command for editor integrations. It does not open Excel, execute VBA, attach to a session, or inspect the workbook VBIDE state. It scans configured standard `.bas` source modules with the Go VBA source pipeline and lists public parameterless `Sub` procedures named `Test*` or `*_Test`, including Unicode VBA identifiers. It excludes class modules, UserForms, workbook document modules, functions, private procedures, and procedures with parameters. `@Tag("name")` comment annotations directly above a test are collected using the same upward scan semantics as workbook-backed test discovery. Successful JSON uses the normal envelope with `command="test list"` and `tests.root`, `tests.summary.files`, `tests.summary.tests`, and `tests.items[]` containing `module`, `name`, `qualified_name`, `source_path`, `line`, and `tags`. `source_path` is project-relative and `line` is 1-based. WSL runs keep `test list` local because it is source-only; workbook-backed `test` remains delegated to Windows.

`test` supports the `.NET` bridge in both explicit `--bridge dotnet` mode and Windows `auto` mode. In `auto`, xlflow uses the `.NET` bridge and does not fall back to PowerShell. The command routes through the `.NET` Excel bridge executable (`xlflow-excel-bridge.exe`) and produces the same envelope fields (`target`, `session`, `workbook`, `tests`, `logs`). The `.NET` bridge implementation handles test discovery (`Test*`/`*_Test`, including Unicode VBA procedure names), `@Tag` annotation collection, lifecycle hooks (`BeforeAll`/`AfterAll`/`BeforeEach`/`AfterEach`), test filtering (by name/module/tag), test runner VBA code generation with per-module hook wrappers, inconclusive detection (`vbObjectError + 516`), runtime injection, UI stream injection, and session-aware workbook attachment. Runtime injection is transactional: if defined-name injection cannot be completed, xlflow rolls back partial injection and does not execute tests.
