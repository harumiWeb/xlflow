...
xlflow [--json] backup prune [--keep-last <n>] [--older-than <duration>] [--max-total-size <size>] [--dry-run] [--all-workbooks] [--include-invalid] [--include-legacy]
xlflow [--json] backup delete --backup <backup-id>
...
xlflow [--json] build [--base <workbook>] [--out <workbook>] [--dry-run]
...
xlflow [--json] test [--filter <name>] [--module <name>] [--tag <tag>] [--isolation none|module|test] [--fail-fast] [--max-failures <n>] [--rerun-failed <n>] [--no-save] [--msgbox <dialog-id=result>]... [--inputbox <dialog-id=value>]... [--filedialog <kind>:<dialog-id>=<value>]... [--ui-stream] [--session]
...
xlflow [--json] impact <Module.Procedure> [--path <dir-or-file>] [--direction callers|callees|both] [--depth <n>]
xlflow [--json] graph dependencies [--path <dir-or-file>] [--module <name>]
...
`new` creates a fresh macro-enabled workbook, Excel add-in, or Excel binary workbook under `build/`, scaffolds the same project layout as `init`, and then automatically `push`es the scaffolded VBA source into that workbook so the initial workbook and `src/` tree start in sync. Without an argument it creates `build/Book.xlsm`; when the argument has no extension, `.xlsm` is appended. Explicit `.xlsm`, `.xlam`, and `.xlsb` filenames are accepted; any other extension is rejected with a validation error. `.xlsm` creation uses Excel file format `52` (`xlOpenXMLWorkbookMacroEnabled`), `.xlam` creation uses file format `55` (`xlOpenXMLAddIn`), and `.xlsb` creation uses file format `50` (`xlExcel12`). Creating an `.xlam` project only writes the add-in file under `build/`; it does not call `Application.AddIns.Add`, mark the add-in installed, copy it into an Office add-ins directory, or otherwise modify persistent Excel add-in configuration. New projects write `[userform].code_source = "sidecar"` into `xlflow.toml`.

The bundled source generated by `new` is expected to produce no findings with the default `xlflow analyze` configuration. This contract covers xlflow-owned scaffold modules; it does not suppress findings in user-authored VBA.

`init` accepts an existing `.xlsm`, `.xlam`, or `.xlsb` path, copies that workbook into the new project's `build/<basename>` path without changing the filename, extension, or workbook format, records that project-local `build/...` path in `xlflow.toml` under `[excel].path` (for example `build/Sales.xlsm`, `build/SalesAddin.xlam`, or `build/LargeModel.xlsb`), and then automatically `pull`s VBA from the copied workbook into `src/` so the initial source tree reflects workbook reality without a second command. Initialized projects write `[userform].code_source = "frm"` so existing `.frm`-embedded code remains authoritative by default. `init --userform-code-source sidecar` explicitly opts into `[userform].code_source = "sidecar"` for imported projects; after the bootstrap pull, xlflow generates `src/forms/specs/<FormName>.yaml` for each imported UserForm using the same non-executing Designer inspection basis as `form snapshot`, while sidecar code is produced by the normal sidecar-aware pull path under `src/forms/code/<FormName>.bas`. Accepted `--userform-code-source` values are `frm` and `sidecar`; any other value fails with `init_failed`. Because this bootstrap pull and sidecar spec generation open Excel/VBIDE, `init` is now an Excel COM-backed command and returns an environment failure if the automatic import cannot complete. `init --with-module` additionally installs `XlflowAssert.bas`, `XlflowRuntime.bas`, `XlflowUI.bas`, and `XlflowDebug.bas` into the configured module source root after the bootstrap pull, then automatically `push`es those helper modules into the copied workbook so source and workbook finish in sync. `init --with-module` refuses to overwrite existing helper source files. The analyzer-clean contract for this option applies to the newly installed xlflow helper modules; imported user VBA remains subject to its own findings.

`new` and `init` create or update a project-local `.gitignore`. The managed entries ignore Excel temporary files (`~$*.xls*`, `*.tmp`), Excel add-ins (`*.xlam`), and xlflow-generated state (`.xlflow/`, `build/`). Existing `.gitignore` content is preserved; missing managed entries are appended without duplicating entries that are already present.
...
`module install` installs the bundled helper modules `XlflowAssert.bas`, `XlflowRuntime.bas`, `XlflowUI.bas`, and `XlflowDebug.bas` under the configured `[src].modules/Xlflow/` directory of an existing xlflow project without changing the workbook by default. `module install --push` additionally imports those helper modules into the configured workbook through the normal `push` path. The command refuses to overwrite target files and also refuses a legacy root-level helper collision, so it never creates two source files for the same VBA component.
...
`lsp --stdio` starts the reusable VBA language server over Language Server Protocol JSON-RPC using standard input and standard output. While the server is running, stdout is reserved exclusively for framed LSP messages; normal logging goes to stderr by default or to `--log-file <path>` when supplied. `--performance-log` is opt-in and writes one structured log line per measured operation to the same logging destination. Performance records include the operation, document URI or path where applicable, document version or generation, input bytes and lines, elapsed time, result count, and outcome; diagnostics records also report whether an obsolete result was discarded, and cache-aware operations may report a cache hit or miss. `textDocument/didOpen` records the synchronous handler duration separately from background analysis. Full diagnostics uses the same enabled-rule capability graph as batch analysis. Concurrent Full requests for one project revision share immutable capability results; a new revision or a change between incomplete and complete project evidence invalidates the previous values. Capability build counters such as `capability_typedb_builds`, `capability_resolution_builds`, `capability_effects_builds`, `capability_array_builds`, `capability_object_builds`, `capability_dataflow_builds`, `capability_dictionary_builds`, `capability_application_state_builds`, `capability_event_reentry_builds`, `capability_public_api_type_index_builds`, and `capability_excel_loop_symbols_builds` are stderr/developer telemetry only and never appear in LSP diagnostic payloads. Workspace-overlay records use `operation="workspaceSymbols/overlay"` and include the overlay generation, elapsed time, outcome, and whether an obsolete result was discarded. `textDocument/didChange/parse` records additionally contain `parse_mode` (`incremental`, `deferred_full`, `full_fallback`, or `retained`) and a `fallback_reason` when applicable. The initial declaration index record uses `operation="workspaceDeclarations/index/initial"`; the semantic workspace index retains `operation="workspaceSymbols/index/initial"`. Both include `file_count`, elapsed time, outcome, and the complete stderr-only counter snapshot. Startup/project-preparation profiling adds stable `stage` records for `workspaceDiscovery`, per-file `declarationIndexing` and `semanticIndexing`, `projectSnapshot`, resolver/view/materialization, effect and constant preparation, `projectChange`, dependency fingerprint updates, and scheduler `permitWait`. Stage and counter records never become LSP response fields; see [`docs/specs/lsp-performance-profiling.md`](./lsp-performance-profiling.md) for the benchmark and pprof procedure. The initial MVP supports `initialize`, `initialized`, `shutdown`, `exit`, `textDocument/didOpen`, incremental `textDocument/didChange`, `textDocument/didClose`, `workspace/didChangeWatchedFiles`, diagnostic publication, `textDocument/documentSymbol`, `workspace/symbol`, `textDocument/definition`, `textDocument/references`, `textDocument/prepareRename`, `textDocument/rename`, `textDocument/hover`, `textDocument/completion`, `textDocument/signatureHelp`, `textDocument/formatting`, `textDocument/semanticTokens/full`, `textDocument/semanticTokens/full/delta`, `textDocument/codeAction`, and `textDocument/codeLens`. The server accepts ordered UTF-16 ranged changes and retains whole-document replacements as a compatibility fallback. Ranged changes build tree-sitter edit coordinates from raw UTF-8 source and parse against an edited clone of the previous snapshot tree; the prior snapshot remains readable until its retirement completes. Unreconcilable coordinates and invalid version ordering retain the last valid snapshot until a full replacement resynchronizes the document. Open editor buffers are authoritative over filesystem content until `didClose`.
...
The LSP recognizes direct `.yaml`, `.yml`, and `.json` files in the configured `src.forms/specs` directory (default `src/forms/specs`) as UserForm specification candidates. Path-based recognition deliberately includes malformed and wrong-`kind` specifications so the editor can report `UFY001` YAML syntax failures and the shared `UFV001`–`UFV014` UserForm contract diagnostics, including unknown/missing fields, type and fixed-value errors, unsupported controls or properties, parent/reference errors, ProgID mismatches, and support-level warnings. YAML diagnostics map unknown and unsupported fields to keys, invalid values and references to scalar values, and fieldless structural failures to the nearest affected mapping or collection. Structured-data files outside that directory are ignored. JSON is detected and reserved for the same future feature path but currently returns no editor features.
...
`form migrate sidecar [FormName]` converts tracked UserForm source artifacts from `frm` code-source mode to `sidecar` mode. It loads `xlflow.toml`, enumerates `.frm` files under `[src].forms` excluding reserved `code` and `specs` directories, optionally filters to one bare form name, extracts code-behind with the UserForm `.frm` splitter, writes `src/forms/code/<FormName>.bas` without exported `Attribute VB_*` header lines, and writes `src/forms/specs/<FormName>.yaml` from non-executing Designer inspection. Because the Designer spec is captured from the configured workbook, the command fails with `form_migrate_conflict` when source files are newer than the workbook; users must `push` source-only edits or `pull` workbook state before migration. It keeps `.frm` and `.frx` artifacts in place and does not mutate `.frx`. Existing sidecar code that differs from the `.frm` code-behind, or any existing spec file, fails with `form_migrate_conflict` unless `--overwrite` is supplied; same-content sidecar code may be reported in `source.skipped[]`. The config switch to `[userform].code_source = "sidecar"` happens only after selected forms are inspected and artifacts are written. Already-sidecar projects run as an idempotent repair/completion flow and leave config unchanged. Successful JSON uses `command="form migrate sidecar"` and `source.operation="userform.migrate_sidecar"`, `source.code_source_before`, `source.code_source_after`, `source.forms[]`, `source.created[]`, `source.updated[]`, `source.skipped[]`, `source.config_path`, and `source.requires_push=false`. Bad arguments fail with `form_migrate_args_invalid`; Designer inspection/spec conversion failures fail with `form_migrate_inspect_failed`; filesystem/config update failures use `form_migrate_failed`.
...
`push` reads source-controlled `.bas`, `.cls`, and `.frm` files as UTF-8 without BOM, writes CP932 temporary import copies under `.xlflow/tmp/`, and imports those temporary files through VBIDE. `.frx` files are binary userform companions and are copied without text conversion. When `[vba.line_numbers].enabled=true`, `push` adds temporary `Erl` instrumentation only to those import copies; it never adds line numbers to tracked source. Eligible procedure statements receive their physical source line number as a fixed-width, space-padded prefix followed by two spaces, without a colon. Before instrumenting, `push` stops safely if it finds numeric `GoTo`, `GoSub`, or `Resume` targets, or existing numeric labels that could be rewritten; there is no line-number CLI flag. Source enumeration is recursive under each configured `[src]` root. In `sidecar` mode, `src/forms/code/*.bas` is reserved for UserForm code-behind sidecars: those files participate in source fingerprinting but are not imported as standalone standard modules. `src/forms/code` is a flat reserved directory, so subdirectories fail source preflight with issue code `FRM203`. Instead, before Excel opens xlflow synchronizes tracked `.frm` embedded code from the sidecar so `src/forms/code/<FormName>.bas` remains authoritative, then after a `.frm` import succeeds it reapplies that sidecar to the imported UserForm `CodeModule`. If a sidecar contains exported UserForm header lines such as `Attribute VB_Name`, preflight fails with `source_preflight_failed` and issue code `FRM202` before Excel opens instead of synchronizing corrupted `.frm` content. In `sidecar` mode, push also validates `src/forms/specs/*.{yaml,yml,json}` against tracked `.frm` artifacts before Excel opens: the spec filename, `form.name`, `.frm` basename, and `.frm` `Attribute VB_Name` must agree, otherwise preflight fails with `source_preflight_failed` and issue code `FRM201` so xlflow cannot import the wrong Designer-backed form. In `frm` mode, `.frm` embedded code remains authoritative and `src/forms/code` is ignored. When `[vba].folder_annotation="update"`, xlflow rewrites `@Folder("A.B")` comments in temporary import copies from the file's relative directory below its configured root; the tracked source file itself is not rewritten during `push`. When `folder_annotation="preserve"`, existing annotations are kept as-is; `ignore` disables folder annotation read/write. Before starting Excel, `push` runs fatal source preflight checks for patterns that are known to surface as VBE modal dialogs instead of COM errors, including typographic quote characters, likely C-style quote escapes, VBA logical lines with more than 24 continuation characters, bare `MsgBox` / `InputBox` calls when `XlflowUI.bas` is present, statically-known object/member mismatches such as `Worksheet.DisplayGridlines`, removed legacy helper APIs such as `XlflowLog` / `XlflowSetTraceFile`, contaminated UserForm sidecars that still contain `Attribute VB_*` export headers, unsupported UserForm sidecar subdirectories, and spec/artifact mismatches that would cause a different UserForm name to be imported than the one declared in `src/forms/specs`. Bare native dialogs should normally be migrated to `XlflowUI.MsgBox` / `XlflowUI.InputBox`; intentional human-only native dialogs must be written as `VBA.Interaction.MsgBox` / `VBA.Interaction.InputBox` so the call target is explicit. Recursive source trees are also validated for duplicate VBA component basenames; conflicts fail with `duplicate_module_name` before Excel opens. These failures return `lint_failed`, `analyze_failed`, `source_preflight_failed`, `duplicate_module_name`, or related environment failures, include top-level `issues` and/or `analysis` when applicable, and use validation exit code `1`. By default (or with `--backup=always`), `push` creates a timestamped workbook backup under `.xlflow/backups/<backup-id>/`, writes `metadata.json` beside the copied workbook file, replaces non-document VBA components, updates document modules, runs VBE Compile, saves the workbook, and writes source fingerprints to `.xlflow/state/push.json`. If VBE Compile fails after import, `push` returns `vba_compile_failed` with `error.phase = "compile_vba"` and validation exit code `1`; the workbook is not saved and source fingerprints are not updated. `.NET` bridge compile failures may include top-level `push_diagnostic.kind = "compile"` with dialog metadata, VBE selection location, and non-fatal `location_capture` attempts. When source mapping is verified, `location.line` and `location.end_line` are source-file line numbers, adjusted for exported metadata such as hidden `Attribute VB_*` lines. `column` and `end_column` are omitted unless the VBE selection exposes a source-column value that xlflow can treat as reliable. Session-backed compile failures also report `session.dirty = true` / `save_required = true` because the live workbook has already received the imported source.

If Excel cannot remove an existing non-document component during replacement, `push` fails before importing source with `vba_component_remove_failed` and `error.phase = "remove_non_document_components"`. The error includes the component name, VBIDE type, kind, and HRESULT. Every imported component name is also checked against its source name; an Excel-assigned suffix fails with `vba_component_import_name_mismatch` before compile or save. Other failures after destructive removal/import begins return `vba_component_replacement_failed` with the active replacement phase. Every session-backed replacement failure reports `session.dirty=true`, `save_required=false`, `discard_required=true`, and `vba_component_replacement_partial`, then poisons the session because the live workbook may already be partially replaced. `session status` continues to report the observed dirty state but does not emit `save_required` for discard-poisoned sessions. For a managed session, `session stop` closes the workbook without saving and reports `discarded_unsaved_changes=true`; a fresh session is required before retrying. An external session is detached without closing its workbook and reports that the unsafe changes were not discarded, so the workbook must be closed in Excel without saving.

`push` supports the `.NET` bridge in both explicit `--bridge dotnet` mode and Windows `auto` mode. In `auto`, xlflow uses the `.NET` bridge and does not fall back to PowerShell. The command routes through the `.NET` Excel bridge executable (`xlflow-excel-bridge.exe`) and produces the same envelope fields (`target`, `session`, `workbook`, `source`, `logs`, and `backup` when a pre-push backup was created). The `.NET` bridge implementation handles standard modules, class modules, UserForms (`.frm` / `.frx`), document modules, backup mode, changed-only fingerprinting, sidecar UserForm code-behind synchronization, and Rubberduck folder annotations.

`push --backup=never` skips the workbook backup. `push --fast` is a development-mode shorthand for `--backup=never --changed-only`. `push --changed-only` compares source fingerprints against `.xlflow/state/push.json`; when unchanged, it skips Excel/VBIDE import and returns `source.changed=false`. When changed or state is missing, v1 safely falls back to the normal full component replacement and refreshes the state file after success. The state file also records where the fingerprinted source was delivered: the session identity (`session_id`, with process id and window handle as fallback) when the push attached to a live session, and the saved workbook file stamp (path, last-write ticks, length) when the push saved the workbook. A changed-only push skips import only when the fingerprint matches _and_ the current target is covered by that delivery record: the same live session (matching session identity, with the recorded Excel instance verified to still hold the workbook open through the same resolution order attach uses), or a target whose disk file still carries the recorded post-save stamp. A fresh managed session therefore never skips solely on a stale project-level cache, while repeated pushes to the same session still skip. Legacy state files without delivery evidence never skip. When a matching session is poisoned, or session resolution would land attach on a different running Excel holding the workbook, the skip is refused so the normal attach path can surface the real session error or import into the live workbook. A skipped push reports the session it verified as the target; because the skip does not attach, its workbook `dirty`/`needs_save` and session `dirty`/`save_required`/`live_newer_than_disk` fields are `null` (unknown) rather than definite values. `push --session` forces attachment to the workbook kept open by `xlflow session start` or adopted by `xlflow session attach`. When `--session` is omitted and `.xlflow/session.json` points at the configured workbook, xlflow auto-reuses that matching live session workbook instead of opening a fresh hidden instance. `push --no-save` is allowed only with `--session` and leaves workbook changes unsaved until `xlflow save` or `xlflow session stop`. Human output must distinguish skipped imports, saved workbook updates, and live-session-only updates that have not yet been written back to disk. When source UserForms are detected, `push` adds top-level `warnings` / `hints` about partial `.frm` fidelity; `push --session --no-save` also adds `userform_unsaved_session_state`.

When `[backup.retention].enabled=true`, successful `push` runs automatic backup pruning after the workbook operation only if a new backup was actually created. Automatic pruning does not run after `--backup=never`, `--fast`, unchanged `--changed-only` no-op pushes, failed pushes, backup creation failures, or config loading failures. Pruning is secondary maintenance: deletion failures do not change a successful push exit code and are reported as warnings.

`backup list` lists the available workbook-file backups for the configured workbook. Each result includes backup `id`, `created_at`, `reason`, original workbook path, backup file path, `size_bytes`, and `status="valid"`. The timestamp comes from backup metadata. Invalid backup directories do not block listing valid backups; they are reported as warnings. Directories without `metadata.json` are treated as legacy entries and are excluded from normal rollback/list targets.

`backup prune` previews or deletes managed backup directories under `.xlflow/backups`. At least one explicit condition or include flag is required; running it without `--keep-last`, `--older-than`, `--max-total-size`, `--include-invalid`, or `--include-legacy` fails with `backup_prune_args_invalid` and must not delete every backup implicitly. `--keep-last <n>` protects the newest `n` metadata-backed valid backups from all other prune conditions. `--older-than <duration>` supports non-negative integer durations with `h`, `d`, or `w`. `--max-total-size <size>` supports decimal `KB`, `MB`, and `GB` units and removes oldest unprotected valid backups until the remaining total is within the limit. By default only backups whose `original_workbook_path` matches the configured workbook are considered; `--all-workbooks` evaluates all valid managed entries. `--include-invalid` and `--include-legacy` explicitly include malformed or metadata-less direct child directories of the backup root. `--dry-run` returns the same candidate set without deleting files and reports `freed_bytes=0`.

Manual `backup prune` options are independent of `[backup.retention]`: manual pruning does not require `enabled=true`. Automatic pruning uses only `[backup.retention]`, is always scoped to the configured workbook, never behaves like `--all-workbooks`, and never deletes invalid or legacy entries.

`backup delete --backup <backup-id>` deletes exactly one metadata-backed managed backup directory for the configured workbook. The selector is a backup ID, not a filesystem path. Missing or empty `--backup` fails with `backup_delete_args_invalid`; unknown IDs fail with `backup_not_found`; backups for another workbook fail with `backup_delete_scope_mismatch`; unsafe deletion targets fail with `backup_delete_unsafe_path`.

Backup deletion uses only directories discovered by the managed scanner. It resolves the backup root and target directory to absolute cleaned paths, rejects deleting the backup root itself, rejects traversal outside the root, rejects ambiguous or non-direct child targets, and checks symlink/junction resolution before deleting. Metadata `backup_file_path` values are not trusted as deletion targets.
...
For `run --headless`, GUI preflight uses the requested macro as the root of a
conservative project call graph. Boundaries owned by procedures that are
definitely unreachable from that root are omitted from the run failure. Static
dynamic targets are treated as possible reachability; unresolved or ambiguous
project dispatch, incomplete parsing, implicit standard-module property
accessors, and boundaries without an identifiable procedure owner retain the
project-wide failure behavior. `inspect-gui`, `lint`, and `doctor` continue to
report the complete project-wide boundary inventory.
...
`test` resolves the runtime execution mode to `test` before user VBA starts and injects the same workbook-scoped runtime markers used by `run`. New scaffolded projects can branch inside tests with `XlflowRuntime.IsTest()` or inspect the string value with `XlflowRuntime.ModeName()`. `test` also accepts the same repeated `--msgbox`, `--inputbox`, and `--filedialog` flags as `run`, plus `--ui-stream` for realtime stderr streaming of resolved `XlflowUI` events while tests are running. `test --visible` is a per-command Excel window override for GUI-bound integration APIs such as `Application.OnKey`: for non-session runs, effective visibility is `[excel].visible || --visible`. It does not change runtime mode, scripted UI handling, or JSON result contracts. With `--session`, the command attaches to the existing Excel instance and does not alter its visibility; managed sessions started by `session start` are already visible. As with `run`, the workbook-scoped marker is the primary contract and `Environ$("XLFLOW_MODE")` remains only a secondary fallback for manual helper adoption in older projects.

`test list --json` is a source-only discovery command for editor integrations. It does not open Excel, execute VBA, attach to a session, or inspect the workbook VBIDE state. It scans configured standard `.bas` source modules with the Go VBA source pipeline and lists public `Sub` procedures named `Test*` or `*_Test`, including Unicode VBA identifiers. Parameterless tests remain one item with `id` and `qualified_name` in `<Module>.<Procedure>` form. Parameterized tests must use one or more directly attached `@TestCase(...)` annotations, and each annotation expands to an independent item with `id` and `qualified_name` in `<Module>.<Procedure>[<case>]` form. `@TestCase("name"; args...)` creates a named case whose `case_id` is `name`; unnamed cases use a lexical canonical form of the literal arguments. Initial canonicalization normalizes whitespace, string escaping, booleans, `Empty`, `Null`, and date delimiters, but does not make semantically equivalent numeric spellings such as `1.0` and `1#` identical. Supported `@TestCase` literals are integer, floating-point/scientific notation, string, `True`, `False`, `Empty`, `Null`, and VBA date literals. Constants, enum members, function calls, member expressions, arrays, object creation, workbook references, object parameters, arrays, `ByRef`, `Optional`, and `ParamArray` are rejected with `error.code="invalid_test_case"`. `@Tag("name")`, `@ExpectedError(...)`, `@Skip[("reason")]`, `@Todo[("reason")]`, and `@TestCase(...)` comment annotations directly above a test are collected using the same upward scan semantics as workbook-backed test discovery. Successful JSON uses the normal envelope with `command="test list"` and `tests.root`, `tests.summary.files`, `tests.summary.tests`, and `tests.items[]` containing `id`, `module`, `name`, `qualified_name`, `qualified_procedure`, `source_path`, `line`, `procedure_line`, `tags`, optional `case_id`, optional `arguments`, optional `annotation_line`, optional `expected_error`, optional `status_hint`, optional `skip`, and optional `todo`. `arguments[]` contains the declared parameter `type` and parsed JSON `value`. `expected_error` contains `number` plus optional `description` and `source`. `status_hint` is `skipped` for `@Skip` and `todo` for `@Todo`; `skip` and `todo` contain optional `reason`. `source_path` is project-relative and lines are 1-based. WSL runs keep `test list` local because it is source-only; workbook-backed `test` remains delegated to Windows.

`test` supports the `.NET` bridge in both explicit `--bridge dotnet` mode and Windows `auto` mode. In `auto`, xlflow uses the `.NET` bridge and does not fall back to PowerShell. The command routes through the `.NET` Excel bridge executable (`xlflow-excel-bridge.exe`) and produces the same envelope fields (`target`, `session`, `workbook`, `tests`, `logs`). The `.NET` bridge implementation handles test discovery (`Test*`/`*_Test`, including Unicode VBA procedure names), stable `id` / `qualified_name` emission, `@Tag`, `@ExpectedError`, `@Skip`, and `@Todo` annotation collection, lifecycle hooks (`BeforeAll`/`AfterAll`/`BeforeEach`/`AfterEach`), test filtering (by qualified name, unique unqualified name, module, and tag), test runner VBA code generation with per-module hook wrappers and one bounded `RunTest_<index>` wrapper per executable test case, inconclusive detection (`vbObjectError + 516`), runtime injection, UI stream injection, and session-aware workbook attachment. Runtime injection is transactional: if defined-name injection cannot be completed, xlflow rolls back partial injection and does not execute tests.
...
Project capability planning adds build counters for the revision-level semantic
builders: `capability_typedb_builds`, `capability_resolution_builds`,
`capability_effects_builds`,
`capability_array_builds`, `capability_object_builds`,
`capability_dataflow_builds`, `capability_dictionary_builds`,
`capability_application_state_builds`, `capability_event_reentry_builds`,
`capability_public_api_type_index_builds`, and
`capability_excel_loop_symbols_builds`. The planner computes transitive
requirements from enabled diagnostics before construction. A required
capability is built at most once per analysis revision; a capability outside
the plan is not built and reports zero. When capability elapsed stages are
enabled, they use the corresponding `capability_*` stage name and the same
revision scope as the build counter. These values are stderr-only telemetry
and do not appear in normal CLI JSON or LSP diagnostic payloads.

The capability graph is internal and does not add a CLI option, configuration
key, diagnostic ID, or public schema field. Resolution-dependent capabilities
...
generated TypeLib data makes external absence unknown and the unresolved branch
fails open; project-local inaccessible and ambiguous types remain reportable.

`check` runs `lint`, `analyze`, then `doctor`. It continues after lint/analyze findings so source issues and environment status are returned together. JSON output includes top-level `check`, `issues`, `analysis`, optional `analysis_metrics`, and doctor diagnostics. Lint/analyze findings with `error`, unknown, or empty severity return exit code `1`; warning- and information-level findings do not change the successful exit code. Doctor/environment failure returns exit code `3`.
...
`impact <Module.Procedure>` is a source-only project-local call-graph query. It never opens Excel. It uses only conservatively `matched` call resolutions; ambiguous, unresolved, external, built-in-like, and dynamic member calls are not turned into dependencies and are summarized under `impact.uncertainty`. The default is `--direction both --depth 1`; `--depth 0` returns only the target. Successful JSON has top-level `impact` with the stable selected target identity, traversal settings, confirmed nodes and call-site edges, direct and transitive callers/callees, affected modules, and detected cycles. Every node and edge includes source location information. `impact_target_not_found` and `impact_target_ambiguous` are validation failures; ambiguity includes every candidate stable identity in `error.details`. Invalid direction/depth arguments are `impact_args_invalid` configuration failures.

`graph dependencies` is a source-only project-local dependency projection and never opens Excel. It reuses the parsed call and symbol facts used by call graph analysis. Successful JSON has top-level `graph` with `target="dependencies"`, stable procedure/module/type nodes, confirmed typed edges (`calls`, `uses_type`, `constructs`, `implements`), source evidence, and separately reported `uncertain_edges`. Module-to-module `calls` edges aggregate their contributing procedure call evidence. `--module` performs a case-insensitive exact source-module filter: its procedures and outbound edges are retained together with referenced target nodes. `--path` restricts the configured source scan in the same way as `impact` and `inspect calls`.
...
HTTP origins used only by `VBA246`. Each value must be an absolute
`http://host[:port]` origin without userinfo, any path (including `/`), query, fragment,
or wildcard; HTTPS entries are invalid. Host casing and IPv6 notation are
normalized, and an explicit default `:80` is removed. `localhost`,
`127.0.0.0/8`, and `::1` are
development origins without configuration. A match suppresses only
...
The staging file and final output share a volume. If the output is absent, bridge publication uses a same-volume atomic move and returns `output.publication="atomic_create"`; if it exists, publication uses the operating system's atomic replace API and returns `output.publication="atomic_replace"`. There is no delete-then-copy fallback: inability to guarantee replacement returns `build_output_replace_failed` without modifying the old output. Successful results include `build.source_applied`, `build.vbe_compile`, `build.workbook_saved`, `build.workbook_closed`, `build.excel_cleanup`, and `output.path`, `output.replaced_existing`, `output.publication`, and `output.temporary_cleanup`. `output.temporary_cleanup` has `status="clean"|"failed"` and, when cleanup fails, may include `residual_path` and `error`.

After a successful non-dry artifact publication, xlflow attempts to publish the versioned companion manifest at `<output>.build.json` through the same staging directory and atomic create/replace semantics. The persisted manifest records `schema_version=1`, command/backend, project-relative base/output and component paths, validation state, and artifact publication metadata. JSON responses expose the corresponding contract under `build`, including `build.publication`, `build.manifest.path`, and `build.manifest.published`. The workbook artifact remains authoritative: if only sidecar publication fails, the command remains successful, preserves the newly published workbook, and emits `build_manifest_publish_failed` with `build.manifest.published=false` and an error detail.
...
`category`, `evidence_class`, `compile_equivalent`, `default_severity`,
`supported_severities`, `surfaces`, `scope`, `precision`, `default_enabled`,
`configurable`, `configuration_key`,
...
- `macros` for `macros`
- `tests` for `test`
- `test_run` for `test`, including `isolation`, `session`, `temporary_workbook`, `source_workbook`, `workbook_saved`, `cleanup` metadata with `status`, plus `path` and `message` when cleanup fails, and `execution` metadata for fail-fast, maximum-failure, and rerun controls
- `diff` for `diff`
- `inspect` for `inspect`
- `issues` for `lint`
...
`test` result objects contain `id`, `qualified_name`, `name`, `module`, `status`, `duration_ms`, `qualified_procedure`, `procedure_line`, `tags`, optional `case_id`, optional `arguments`, optional `annotation_line`, optional `reason`, optional `expected_error`, optional `observed_error`, optional `error`, optional `attempts`, optional `flaky`, and optional `attempt_results`. `tags` is always present. `arguments[]` contains the declared parameter `type` and parsed JSON `value` for parameterized cases. `reason` is present for skipped/todo tests when the annotation supplied a reason and for `not_run` tests when early termination prevented execution. `expected_error` contains `number` plus optional `description` and `source`. `observed_error` contains the observed VBA `number`, `source`, and `message` when an expected-error test body raised an error. Executed tests include `attempts`, `flaky`, and `attempt_results`; each attempt result contains `attempt`, `status`, `duration_ms`, and optional `error`.

`status` values are `passed`, `failed`, `skipped`, `todo`, `inconclusive`, and `not_run`. `skipped` and `todo` are produced from non-executed `@Skip` / `@Todo` metadata. `inconclusive` is produced when a test body executes and calls `XlflowAssert.AssertInconclusive`. `not_run` is produced only when early termination prevents a selected test from being scheduled.
...
`backup list` success results add top-level `backups`. `backups` is an array of valid backup records. Each record contains `id` (string), `created_at` (ISO 8601 timestamp from metadata), `reason` (string such as `before-push` or `pre-rollback`), `workbook` (project-relative or absolute workbook path), `path` (project-relative or absolute backup workbook path), `size_bytes` (integer backup file size), and `status="valid"`. Malformed backup directories are excluded from `backups` and reported via top-level `warnings` with `code="invalid_backup_entry"`, `path`, and diagnostic `reason` / `detail`. Legacy directories without `metadata.json` are excluded from normal output.

`backup prune` success and partial-failure results add top-level `backup_prune` with `dry_run`, `matched`, `deleted`, `failed`, `freed_bytes`, `candidates`, `deleted_entries`, `failed_entries`, `skipped_invalid_entries`, and `skipped_legacy_entries`. Dry-run responses keep matched items in `candidates` and leave `deleted_entries` empty. Real prune responses move successful deletions to `deleted_entries`; each deleted entry contains `id`, `path`, and `freed_bytes`. Candidate entries contain `id`, `path`, `size_bytes`, `status`, `reasons`, and, for valid backups, metadata `created_at` and `reason`. Partial deletion failures are reported in `failed_entries` with `id`, `path`, `code`, and `message`; the command may exit non-success while preserving successful deletion details in the same JSON payload. Automatic prune summaries reuse this field with `automatic=true` and may appear on successful `push` or `rollback` results when entries were deleted, deletion failed, or invalid/legacy entries were skipped.

`backup delete` success results add top-level `backup_delete` with `id`, `path`, and `freed_bytes` for the deleted backup directory.
...
- `0`: success
- `1`: user-code or validation failure, including error-severity, unknown, or empty-severity lint/analyze findings (including source diagnostics reported by `check`), metrics threshold findings (`metrics_threshold_exceeded`), hotspot score-threshold findings (`metrics_hotspot_threshold_exceeded`), GUI boundary preflight failures, macro failure, macro timeout, VBE compile failure, missing macro target, removed-helper findings, missing UI sheets or buttons, VBA test failure, no tests found, missing or ambiguous filter targets, active workbook mismatches, duplicate test names within one module, invalid exported ranges, existing output files, unsupported export-image formats, unsupported form export-image formats, missing UserForms, `form_already_exists`, `unsupported_form_control`, `designer_write_failed`, capture window lookup failures, image capture failures, `edit` session requirements, invalid workbook edit selectors, invalid edit colors, `invalid_sheet_name`, `sheet_exists`, `fmt_check_failed` (unformatted files in `--check` mode), and workbook event-handler failures returned by the bridge
...
- `VB022`: legal but confusing parenthesized call syntax such as `Foo (bar)`;
  it remains a maintainability warning and does not claim that VBE rejects the
  source
...
- `VB048`: a procedure parameter declaration violates a VBA compile-time
  contract, such as an invalid array shape, `ParamArray` placement or type,
  required parameters after `Optional`, an unsupported UDT passing mode, an
  excessive parameter count, or a definitely invalid optional default value.
  Structural violations remain reportable when type resolution is incomplete;
  type-dependent compatibility checks are fail-open for unknown or ambiguous
  types. The parameter (or procedure name for a parameter-count violation) is
  the primary issue range, and the finding is an unsuppressible error that
  blocks source preflight.
- `VB049`: Property Get/Let/Set accessors with the same module-local name do
  not form a compatible VBA accessor contract. The check validates setter value
  parameters and return declarations, indexed-parameter shape/order and
  passing metadata, and Get return type versus the setter value type. Let and
  Set value types are checked against the VBE's value/object compatibility
  rules when the type is resolved; unknown or ambiguous types are fail-open.
  Cross-accessor findings point at the later inconsistent accessor and are
  unsuppressible errors that block source preflight.
- `VB050`: a declaration uses a module context that VBA rejects. `Event`,
  `Implements`, `Friend`, and `WithEvents` are checked against the canonical
  standard/class/document/UserForm kind. Object modules additionally reject
  public `Const`, fixed-length `String`, arrays, `Type`, and non-`Private`
  `Declare` members. `WithEvents` must be a module-level, scalar, explicitly
  event-capable object without `As New`; unresolved or ambiguous type metadata
  remains fail-open. The issue `kind` identifies the specific contract (for
  example `invalid_event_module` or `invalid_withevents_shape`), and the
  declaration keyword/declarator is the primary range. This is a
  compile-equivalent, unsuppressible error that blocks source preflight.
- `VB051`: the `Me` token occurs in a standard module. Only an AST `Me` token
  is considered; comments, strings, attributes, and other `Me`-related forms
  are ignored. The token is the primary range and the finding is an
  unsuppressible compile-equivalent error that blocks source preflight.
- `VB052`: a call resolves to a project-local target that is provably missing
  or known to be non-callable (for example, a scalar, `Const`, `Type`, Enum
  member, or Event). Bare names that could be external, late-bound, dynamic,
...
- `VB054`: `RaiseEvent` names an event that is not declared in the same object
  ...
  range.
- `VB059`: compile-equivalent call syntax error. It reports explicit `Call`
  statements whose arguments are not parenthesized, empty or multi-argument
  parentheses on standalone calls, Function calls used in expressions without
  required parentheses, and explicit `Call` targets that the AST identifies as
  invalid. Valid ByRef/ByVal parenthesized argument idioms such as
...
- `VB037`: value assignment incorrectly uses `Set`; the VBE rejects it, so it
  is an error and blocks source preflight
...
`VBA250` is also non-blocking. It reports a warning when a `Worksheet.Select`
or `Range.Select` operation lacks a procedure-local proof that its required
workbook or worksheet is active, while the `runtime-safety` evidence class
keeps intentional UI navigation separate from compile-equivalent diagnostics.
...
- `VBA230`: Dictionary `.CompareMode` is changed after the same identity definitely received an entry
- `VBA231`: Dictionary `.Keys` or `.Items` is repeatedly materialized inside a loop
- `VBA232`: accesses derived from the same Dictionary key source use inconsistent explicit normalization
- `VBA233`: a late-bound Dictionary depends on an undefined Scripting comparison constant
- `VBA234`: a `For Each` loop mutates the same Collection identity it is enumerating
- `VBA235`: a zero-based index is used directly with a one-based Collection
- `VBA236`: a process-launch command may combine external input, an unquoted executable path, a command-line secret, or unobserved execution
- `VBA238`: a loop repeatedly resolves an invariant Excel object-model member chain that can be cached outside the loop
- `VBA239`: a SQL statement may combine external input, dynamic identifiers, locale-sensitive values, manual quoting, or wildcard input before execution
...
- `VBA253`: a known implicit, indexed, or recursive default-member access is used where the member can be made explicit
...
- `VBA260`: a `ThisWorkbook.Worksheets("name")` access uses a visible sheet name when an authoritative workbook catalog resolves a stable CodeName (opt-in, batch-only)
- `VBA261`: a generated-TypeLib worksheet-function member is dispatched through `Excel.Application` instead of the explicit `WorksheetFunction` object (opt-in)
- `VBA262`: a bare Excel host bracket expression such as `[A1]` hides its object-model binding (opt-in)
- `VBA265`: a private-procedure parameter is never referenced in the procedure body (opt-in)
- `VBA266`: a module-level `Private Const` is never referenced in the module (opt-in)
- `VBA267`: a `Private Type` member is never accessed through a resolvable member expression (opt-in)
- `VBA268`: a procedure-local scalar variable is read but never assigned (opt-in)
- `VBA269`: a procedure-local variable is read on a path where no assignment is guaranteed (opt-in)
...
`VBA265` maps to `detect_unused_parameters`; `VBA266` maps to
`detect_unused_private_constants`; `VBA267` maps to
`detect_unused_udt_members`; `VBA268` maps to
...
`VBA278` maps to `detect_public_member_underscore_names`; `VBA279` maps to
`detect_document_module_public_enum`; `VBA280` maps to
`detect_write_only_property`; `VBA281` maps to
`detect_public_interface_event_members`; and `VBA282` maps to
`detect_predeclared_instance_access`.
`VBA283` maps to `detect_invalid_ismissing_usage`.
...
`VBA253`, `VBA254`, and `VBA255` are opt-in through their dedicated
configuration keys.

`VBA260`, `VBA261`, and `VBA262` are opt-in, high-precision, non-blocking,
inline-suppressible procedure-local inspections. `VBA260` is a warning-level,
batch-only rule because it requires authoritative workbook-instance metadata;
filesystem analysis derives that metadata from the configured saved OOXML
workbook, while `AnalyzeProject` requires explicit capability injection and
otherwise fails open with a structured warning. `VBA261` is information-level
and available in batch, realtime, and LSP analysis only with a complete
generated `Excel.WorksheetFunction` member set and a receiver resolved exactly
to `Excel.Application`. `VBA262` is warning-level and available in batch,
realtime, and LSP analysis; it uses raw procedure IR to distinguish host
bracket expressions from array/index syntax and qualified bracketed members.
`VBA270` is an opt-in, warning-level, non-blocking, inline-suppressible
file-local inspection available in batch, realtime, and LSP analysis; it
reports worksheet-visible `Function` declarations in standard modules whose
names parse as valid A1 or absolute R1C1 cell references within the current
worksheet limits. Their detailed contracts are defined in
[Excel semantic inspections](excel-semantic-inspections.md).

`VBA256` is an opt-in, warning-level, non-blocking, inline-suppressible
...
`VBA265` through `VBA269` are opt-in, non-blocking, inline-suppressible rules
available in batch and realtime analysis. `VBA265`, `VBA266`, `VBA268`, and
`VBA269` are warning-level; `VBA267` is information-level. Enable them with
...
`VBA278` through `VBA282` are opt-in, warning-level, non-blocking,
inline-suppressible rules for VBA class and interface public-API hazards,
available in batch, realtime, and LSP analysis. `VBA278`, `VBA280`, and
`VBA281` read procedure-symbol metadata on class, form, and document
modules; `VBA279` reads module declarations on document modules only; and
`VBA282` reports self-name references inside modules with a predeclared
default instance. Recovered symbols, conditional-compilation members,
recognized event handlers, unverified or implemented-interface bindings,
private implementations, and type-position self-name references fail open
...
when it has an explicit default; members, locals, other expressions, typed or
required parameters, and arrays are reported. Unresolved, shadowed, malformed,
recovered, and conditionally compiled calls remain silent. Its contract is in
[VBA IsMissing Diagnostics](vba-ismissing-diagnostics.md).
...
Complete default-member failures add runtime kinds
`default_member_required` and `default_member_cycle`; these remain non-blocking
...
`VBA250` is default-enabled, warning-level, medium-precision,
procedure-local, available in batch and real-time analysis, inline-suppressible,
and non-blocking. It tracks workbook and worksheet active-state facts within a
single procedure. `Workbook.Activate` establishes the workbook, while
`Worksheet.Activate` establishes both the parent workbook and worksheet;
`Worksheet.Select` requires the parent workbook and `Range.Select` requires the
owning worksheet to be known active on every reachable path. Conditional
activation, unknown calls, and helper-procedure effects remain conservative.
Use `detect_unsafe_select_operations`, `[analyze].disabled_rules = ["VBA250"]`,
or a line suppression for intentional UI navigation. `VBA250` does not replace
the independent `VB003` style diagnostic.

`VBA251` is default-enabled, warning-level, high-precision, procedure-local,
available in batch and real-time analysis, inline-suppressible, and
non-blocking. It reports a typed Excel `Match`, `VLookup`, or `HLookup` call
when the optional match-mode argument is omitted. Explicit exact or approximate
values, including named arguments and variables, are accepted; unresolved,
late-bound, user-defined same-name calls, `Lookup`, and `XLookup` remain silent.
The rule makes source intent explicit and does not determine data ordering or
...
`VBA253` is disabled by default, warning-level, high-precision, procedure-local,
available in batch, realtime, and LSP analysis, inline-suppressible, and
non-blocking. It reports known implicit, indexed, or recursive default-member
access and adds an additive `default_member` context with `kind`, `binding`,
`expected_context`, `member`, and `depth`. Use
...
`VBA254` is disabled by default and becomes an information-level,
medium-precision, procedure-local diagnostic when
`detect_unbound_default_member_access` is enabled. It reports unbound,
late-bound, or incomplete default-member access and may use `warning` when the
configured policy escalates it. `VBA255` is likewise disabled by default and
uses `detect_bang_notation` to report high-precision information/warning
diagnostics for `receiver!name` bang notation. Both are non-blocking,
inline-suppressible, and available in batch, realtime, and LSP analysis.
Incomplete, stale, curated-only, ambiguous, `Object`, `Variant`, and dynamic
type information never supports `VBA253` or `VBA249`; when enabled, `VBA254`
may report that binding uncertainty as advisory evidence.

Complete default-member failures remain owned by `VBA249` as non-blocking
runtime-error evidence with kinds `default_member_required` and
`default_member_cycle`. `VBA202` retains
exclusive ownership of proven error-91 object-use-before-`Set`/`Nothing`
cases.
...
lost. Ambiguous, unresolved, external, and dynamic calls preserve uncertainty
without creating a definitive finding. Intentional rethrow, explicit Boolean
failure returns, explicit fallback returns, explicit `Resume`, and narrow
checked probes are accepted. The detailed contract is defined in [VBA Error
Suppression and Propagation Analysis](vba-error-propagation-analysis.md).

`VBA238` is warning-only, non-blocking, inline-suppressible, and available in
batch and real-time analysis. It compares normalized Excel member chains inside
reachable loops and reports repeated chains whose constant receivers and
arguments do not depend on the active loop variable. Constant workbook,
worksheet, table, named-range, pivot-table, and chart lookups are eligible;
iterator-dependent expressions such as `Cells(i, 1)` and trivial local-variable
access are not. The finding suggests extracting the chain into a local object
before the loop. Disable it with `[analyze].disabled_rules = ["VBA238"]` or
the compatibility key `detect_loop_invariant_excel_object_resolution`.

`VBA239` reports procedure-local SQL construction that combines external input,
dynamic identifiers, locale-sensitive values, manual quoting, or wildcard input
before execution. It is warning-only, non-blocking, realtime, and inline-
suppressible; disable it with `[analyze].disabled_rules = ["VBA239"]` or use
the corresponding inline suppression comment.

`VBA241` defaults to warning, is non-blocking and inline-suppressible, and is available in
batch and real-time analysis. It reuses the existing `ReDim Preserve` parser
and dimension analysis to report reachable resizes in every supported `For`,
`For Each`, `While`/`Wend`, and `Do`/`Loop` form. A single non-nested loop whose
dimension expressions are independent of loop variables is classified as a
repeated constant-size reallocation and may use `information`; a dimension
that references the current or an outer loop variable, or any finding nested
at depth two or greater, uses `warning`. The message distinguishes the two
cases and recommends preallocation or geometric capacity growth. Fixed-array
and scalar targets remain with the existing allocation/correctness rules, and
non-final-dimension validity remains owned by `VBA208`. Disable it with
`[analyze].disabled_rules = ["VBA241"]` or the compatibility key
`detect_redim_preserve_in_loops`.

`VBA242` is disabled by default and becomes an information-level,
non-blocking, inline-suppressible diagnostic in batch and real-time analysis
when `detect_expensive_full_range_operations` is enabled. It recognizes
full-row, full-column, full-sheet, and direct or unbounded `UsedRange` shapes
only when they feed formula/value assignment, calculation, formatting,
find/replace, or sorting operations. Explicit bounded ranges, bounded
`Resize`, and bounded `Intersect` forms are accepted. A reachable loop
escalates the finding to `warning`; outside loops it remains `information`.
Disable it explicitly with `[analyze].disabled_rules = ["VBA242"]`, and
suppress an intentional one-time whole-range operation with
`xlflow:disable-line VBA242` or `xlflow:disable-next-line VBA242`.

`VBA243` is disabled by default and becomes an information-level,
non-blocking, inline-suppressible diagnostic in batch and real-time analysis
when `detect_value2_performance_opportunities` is enabled. It reports only
bulk or repeated `Range.Value` transfers with a strong performance signal and
does not recommend `Value2` when Date or Currency coercion appears intentional.
...
inline-suppressible. It reports unsafe destructive and state-dependent file
paths, including empty/root, relative/current-directory, wildcard, traversal,
unchecked-overwrite, same-source/destination, external-input, and missing
temporary cleanup risks. Its additive `file_operation` context identifies the
operation, path role, risk class, risk kind, origin, trusted anchor, and known
overwrite policy. Disable it with
`[analyze].disabled_rules = ["VBA245"]`; when disabled, `VBA224` remains the
compatibility fallback for its legacy destructive-file and SaveAs sinks;
non-destructive workbook-open flows remain under `VBA224`.

Its legacy configuration key is `detect_unsafe_file_path`.

`VBA246` and `VBA247` are default-enabled, warning-level, procedure-local,
realtime, non-blocking, and inline-suppressible. `VBA246` recognizes common
early- and late-bound XMLHTTP, ServerXMLHTTP, and WinHTTP identities and reports
plain-HTTP credential use, credentials in URLs, authorization logging,
certificate-validation bypass, obsolete TLS protocols, sensitive module
constants used by HTTP, and an HTTP response saved through ADODB.Stream and
launched from the same executable/script path. `VBA247` independently reports
ServerXMLHTTP and WinHTTP `Send` operations that lack a timeout on every
reaching path or use a known `0`/`-1` unbounded component; dynamic values and
XMLHTTP remain unclassified.

The additive `http_security` and `http_reliability` JSON contexts expose only
API/classification metadata, header names, credential-free origins, and timeout
state. They never expose header values, URL userinfo, path, query, or fragment;
credential-bearing findings replace string literals and comments in
`nearby_code` with `[REDACTED]`. When `VBA246` owns the same HTTP source/sink,
the generic `VBA224` projection is suppressed. Disabling `VBA246` restores the
existing generic fallback. See [HTTP Transport Security Analysis](http-transport-security-analysis.md).
