...
`analyze` scans configured source directories without Excel COM for runtime-risk patterns. Its internal procedure-effect summary follows uniquely resolved, reachable project-local calls; ambiguous, unresolved, external, and dynamically bound calls do not establish propagated effects. `VBA203` prefers a matching same-module Push/Pop or Push/Restore helper pair and may use a direct or propagated restore from either same-module alias. A cross-module pair is accepted only when exactly one candidate is a non-private procedure in a standard module; class, document, and UserForm procedures do not establish a receiver-less cross-module pair. Ambiguous or missing pairs remain findings. Batch-only `VBA221` adds one caller-context warning per Application property when a direct resolved callee has a direct `VBA203` leak origin; it names the origin and callee uncertainty but does not repeat a transitive leak at ancestor callers. Batch-only `VBA222` checks public function/property return types, parameters, and custom event parameters against project declarations plus the embedded/generated TypeLib database. Standard modules and `VB_Exposed=True` classes/interfaces are public API surfaces; private or unexposed project types, ambiguous types, and unresolved external types are reported conservatively. Host-required event handlers are excluded. Opt-in `VBA240` indexes module-level mutable state across standard, class, document, and UserForm modules, reports structural lifecycle coupling, and adds informational `analysis_metrics.module_state` when enabled; fan-in counts alone do not trigger a finding. It returns top-level `analysis`, optional `analysis_metrics`, and findings containing `code`, `severity`, `file`, `module`, `procedure`, `line`, `message`, `reason`, `suggestion`, and `nearby_code`. Findings are validation failures with exit code `1`; analysis metrics do not affect the exit code.
`analyze` scans configured source directories without Excel COM for runtime-risk patterns. Its internal procedure-effect summary follows uniquely resolved, reachable project-local calls; ambiguous, unresolved, external, and dynamically bound calls do not establish propagated effects. `VBA203` prefers a matching same-module Push/Pop or Push/Restore helper pair and may use a direct or propagated restore from either same-module alias. A cross-module pair is accepted only when exactly one candidate is a non-private procedure in a standard module; class, document, and UserForm procedures do not establish a receiver-less cross-module pair. Ambiguous or missing pairs remain findings. Batch-only `VBA221` adds one caller-context warning per Application property when a direct resolved callee has a direct `VBA203` leak origin; it names the origin and callee uncertainty but does not repeat a transitive leak at ancestor callers. Default-enabled `VBA224` performs conservative, procedure-local source-to-sink analysis for non-process APIs and adds optional `data_flow` source/sink/path context to its finding. Default-enabled `VBA236` owns process-launch safety for VBA `Shell`, `WScript.Shell.Run`/`Exec`, `Shell.Application.ShellExecute`, and Win32 `ShellExecute` variants; it adds optional `command_execution` context with risk class/kind, launcher/interpreter, command role, and origin state. `VBA238` reports repeated loop-invariant Excel member-chain resolution and suggests hoisting the chain into a cached local. Unknown origins remain general process-launch risk rather than a confirmed injection claim. It returns top-level `analysis`; findings contain `code`, `severity`, `file`, `module`, `procedure`, `line`, `message`, `reason`, `suggestion`, `nearby_code`, and rule-specific additive contexts. Findings are validation failures with exit code `1`.
`analyze` scans configured source directories without Excel COM for runtime-risk patterns. Its internal procedure-effect summary follows uniquely resolved, reachable project-local calls; ambiguous, unresolved, external, and dynamically bound calls do not establish propagated effects. `VBA203` prefers a matching same-module Push/Pop or Push/Restore helper pair and may use a direct or propagated restore from either same-module alias. A cross-module pair is accepted only when exactly one candidate is a non-private procedure in a standard module; class, document, and UserForm procedures do not establish a receiver-less cross-module pair. Ambiguous or missing pairs remain findings. Batch-only `VBA221` adds one caller-context warning per Application property when a direct resolved callee has a direct `VBA203` leak origin; it names the origin and callee uncertainty but does not repeat a transitive leak at ancestor callers. `VBA225` runs in batch and shared real-time analysis and reports confirmed cell-by-cell Excel object-model reads, writes, formulas, formatting, lookups, worksheet-function calls, and uniquely resolved helper effects inside non-trivial loops; nested-loop context is retained in the message but does not escalate warning severity, while bulk range transfers and statically provable loops of three or fewer iterations are exempt. `VBA238` runs in batch and shared real-time analysis and reports repeated loop-invariant Excel member-chain resolution that can be hoisted into a cached local; expressions depending on the loop variable are ignored. `VBA239` owns procedure-local SQL construction safety for statements that combine external input, dynamic identifiers, locale-sensitive values, manual quoting, or wildcard input before execution. `VBA228` reports only definite project-local ByRef type or array-shape mismatches rejected by the VBE; it is an unsuppressible error and blocks source preflight. Opt-in `VBA240` also indexes module-level mutable state in batch mode and returns optional `analysis_metrics.module_state`; confirmed lifecycle coupling can produce warning findings, while fan-in counts alone do not. It returns top-level `analysis` and optional `analysis_metrics`; findings contain `code`, `severity`, `file`, `module`, `procedure`, `line`, `message`, `reason`, `suggestion`, and `nearby_code`. Findings are validation failures with exit code `1`; metrics do not affect the exit code.
