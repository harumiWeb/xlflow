...
After a successful non-dry artifact publication, xlflow writes the versioned companion manifest at `<output>.build.json` through the same staging directory and atomic create/replace semantics. The persisted manifest records `schema_version=1`, command/backend, resolved base/output, included/excluded components, validation state, and artifact publication metadata. JSON responses expose the corresponding contract under `build`, including `build.publication`, `build.manifest.path`, and `build.manifest.published`. The workbook artifact remains authoritative: if only sidecar publication fails, the command remains successful, preserves the newly published workbook, and emits `build_manifest_publish_failed` with `build.manifest.published=false` and an error detail.
