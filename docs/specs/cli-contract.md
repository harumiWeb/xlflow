...
`build` defaults its base workbook to `[excel].path` and its output to `build/Release/<base filename>`. The base must exist and use `.xlsm`, `.xlam`, or `.xlsb`; the output uses the same format and is a complete project-local file path, not a directory. `build --dry-run` never opens Excel, creates directories, or writes an artifact, and emits `build.dry_run`, `build.base`, `build.output`, `build.included`, `build.excluded`, and `build.warnings`. The current contract-only release returns `build_not_implemented` for a validated invocation without `--dry-run`; temporary-copy mutation, VBE compilation, and output publication will be added by the later pipeline implementation.
