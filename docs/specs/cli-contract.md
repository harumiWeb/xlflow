...
- `VBA228`: definite ByRef type or array-shape mismatch rejected by the VBE; the VBE-accepted low-level reinterpretation of a local user-defined-type array as a pointer-sized intrinsic array is excluded structurally, without library-specific type-name exceptions. It is an unsuppressible error and blocks source preflight, and remains enabled even when `VBA206` is disabled.
