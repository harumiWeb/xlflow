...
- `VBA270`: an `Array(...)` call resolved to the VBA intrinsic returns a zero-based array despite `Option Base 1` (opt-in)
...
`detect_unassigned_variable_usage`. `VBA270` maps to
`detect_option_base_array_inconsistency`; and `VBA271` maps to
