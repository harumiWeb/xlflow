...
Analyzer rules `VBA201` through `VBA206`, `VBA208`, `VBA209`, `VBA211`, `VBA212`, `VBA214` through `VBA227`, `VBA230` through `VBA237` are enabled by default. `VBA230` through `VBA237` are warning-level, non-blocking, and inline-suppressible. `VBA237` is interprocedural and is published by LSP Full diagnostics only. `VBA222` is a batch-only, warning-level, non-blocking rule; it checks public function/property return types, all public parameters, and custom event parameters. Intrinsic types and types resolved from the project or available TypeLib database are allowed. Private/unexposed project types, ambiguous names, and unresolved external types are conservative warnings that include the type name; host-required event handlers are excluded. It can be suppressed inline or with `[analyze].disabled_rules = ["VBA222"]`.
