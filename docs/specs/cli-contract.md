...
xlflow is a Windows-first Go CLI that treats Excel VBA projects as source-controlled code. Excel operations use the `.NET` Excel bridge by default on Windows. The PowerShell bridge is deprecated in v0.15.0, remains available only as an explicit opt-in compatibility path, and is planned for removal in v0.16.0. WSL is supported as a development frontend by delegating Excel-related commands to Windows `xlflow.exe`. Source-only commands such as `lint` remain local and testable without Excel installed.
