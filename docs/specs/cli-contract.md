...
Excel COM-backed commands always report in-flight progress on stderr. Interactive terminals may show Bubble Tea/Lipgloss spinner output, and `--json` or non-interactive runs keep the same progress path on stderr so stdout remains a single final human result or JSON envelope. Agents should wait for process exit instead of parsing interim stderr progress text.
