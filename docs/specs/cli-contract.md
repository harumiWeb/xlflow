...
`trace` supports the `.NET` bridge in both explicit `--bridge dotnet` mode and Windows `auto` mode. In `auto`, xlflow prefers the `.NET` bridge and falls back to PowerShell only when `.NET` is unavailable, incompatible, or unsupported for the command. The command routes through the `.NET` Excel bridge executable (`xlflow-excel-bridge.exe`) and produces the same envelope fields (`target`, `session`, `workbook`, `trace`, `source`, `logs`). The `.NET` bridge implementation handles `enable`, `disable`, `status`, and `clean` with the same behavior as the PowerShell path, including source-match safety for `disable` and automation macros force-disable for direct-open non-executing paths. `trace disable` validates a tracked `XlflowTrace.bas` before removing the workbook helper, so `trace_source_modified` failures do not mutate the live workbook.
