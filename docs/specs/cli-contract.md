...
The LSP also recognizes direct `.yaml`, `.yml`, and `.json` files in the configured `src.forms/specs` directory (default `src/forms/specs`) as UserForm specification candidates. A parseable `kind` must be `xlflow.userform`; files with another kind and structured-data files outside that directory are ignored. YAML documents preserve mapping keys, scalar values, sequence indexes, UTF-16 ranges, and degraded indentation-based cursor context while editing. The initial slice publishes only YAML syntax diagnostic `UFY001`; UserForm contract diagnostics, completion candidates, and Hover documentation remain unavailable until their dedicated follow-up work. JSON is detected and reserved for the same future feature path but currently returns no editor features.
