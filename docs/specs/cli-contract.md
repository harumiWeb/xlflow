...
`build` defaults its base workbook to `[excel].path` and its output to `build/Release/<base filename>`. The base must exist and use `.xlsm`, `.xlam`, or `.xlsb`; the output uses the same format and is a complete project-local file path, not a directory. `build --dry-run` never opens Excel, creates directories, writes an artifact, acquires workbook coordination, or delegates from WSL; it emits `build.dry_run`, `build.base`, `build.output`, `build.included`, `build.excluded`, and `build.warnings`. A non-dry build acquires workbook coordination, rejects a dirty matching xlflow session, reconstructs an isolated staging workbook, requires VBE compilation, saves and closes the workbook, confirms Excel cleanup, and atomically replaces the output only after all validation stages pass. Failures leave an existing output untouched; uncertain Excel cleanup publishes workbook recovery quarantine. Successful results include `build.source_applied`, `build.vbe_compile`, `build.workbook_saved`, `build.workbook_closed`, `build.excel_cleanup`, and `output.replaced_existing`.
