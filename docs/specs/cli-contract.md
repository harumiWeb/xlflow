...
`build` defaults its base workbook to `[excel].path` and its output to `build/Release/<base filename>`. The base must exist and use `.xlsm`, `.xlam`, or `.xlsb`; the output uses the same format and is a complete project-local file path, not a directory. `build --dry-run` never opens Excel, creates directories, writes an artifact, acquires workbook coordination, or delegates from WSL; it emits `build.dry_run`, `build.base`, `build.output`, `build.included`, `build.excluded`, and `build.warnings`.
