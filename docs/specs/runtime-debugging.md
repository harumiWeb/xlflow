...
reliability diagnostic. It reports every unique directed project-local
procedure cycle with a deterministic closed path; ordinary cycles use
`information`, while event handlers, Application-state changes, error
suppression, workbook acquisition, or VBA file acquisition elevate the finding
to `warning`. Its additive `call_cycle` JSON context preserves path, edge,
effect, and uncertainty provenance.
