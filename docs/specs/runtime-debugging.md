...
Excel object-model work, while `VBA238`
(`detect_loop_invariant_excel_object_resolution`) reports repeated resolution
of an invariant Excel member chain that can be cached before the loop. Both
rules are warning-only, non-blocking, inline-suppressible, and available in
