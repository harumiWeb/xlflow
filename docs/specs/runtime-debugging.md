...
Excel object-model work, `VBA238`
(`detect_loop_invariant_excel_object_resolution`) reports repeated resolution
of an invariant Excel member chain that can be cached before the loop, and
`VBA241` (`detect_redim_preserve_in_loops`) reports repeated `ReDim Preserve`
copies inside reachable loops. `VBA225` and `VBA238` are warning-only;
`VBA241` uses `information` for a single non-nested loop with loop-invariant
dimensions and `warning` for loop-variable growth or nested loops. All three
are non-blocking, inline-suppressible, and available in batch and real-time
analysis. Opt-in `VBA242`
(`detect_expensive_full_range_operations`) reports expensive formula/value,
calculation, formatting, find/replace, or sorting operations over entire rows,
...
`VBA244` (`detect_procedure_call_cycles`) is a default-enabled, batch-only
reliability diagnostic. It computes cyclic strongly connected components (SCCs)
from uniquely resolved project-local calls and emits one deterministic closed
representative witness per SCC. An SCC is cyclic when it contains multiple
procedures or a self-edge; normal analysis does not enumerate elementary cycles.
Ordinary components use `information`, while event handlers, Application-state
changes, error suppression, workbook acquisition, or VBA file acquisition
elevate the finding to `warning`. Its additive `call_cycle` JSON context
preserves the representative path and edge data, while aggregating effect,
module-boundary, event, and uncertainty provenance from the full SCC.
...
Typed `Excel.WorksheetFunction` member availability is reported by
...
and `VBA255` is an opt-in information/warning diagnostic for bang notation
(`receiver!name`). These findings add a `default_member` JSON context with the
access kind, binding, expected context, resolved member, and chain depth.
Complete default-member failures are projected through `VBA249` with runtime
kinds `default_member_required` or `default_member_cycle`; proven error-91
object-use-before-`Set`/`Nothing`
remains owned by `VBA202`.

`xlflow analyze` scans configured source directories without Excel COM. The analyzer uses `tree-sitter-vba` to build per-file and per-procedure context from declarations, parameters, function and property returns, labels, assignments, calls, and member access. It also builds internal direct and propagated procedure-effect summaries from uniquely resolved, reachable project-local calls; unresolved or dynamically bound calls remain uncertain rather than proving an effect. It detects likely missing `Set` assignments for object variables and object-returning functions, statically-known Excel object/member mismatches, missing valid return assignments for `Function` and `Property Get` procedures on reachable CFG paths, removed legacy helper APIs such as `XlflowLog` and `XlflowSetTraceFile`, `Range.Find` results used without `Nothing` guards, object variables used before obvious initialization, object `Nothing` guards combined with dereferences in non-short-circuit boolean expressions, Application state leaks, error-handler fallthrough, leaked `On Error Resume Next` scopes, unqualified Excel object access, Dictionary and Collection guard/order/iteration/index mistakes, unhandled exceptional Excel API calls, event-handler re-entry hazards, public API type visibility hazards, unsafe `Range.Value` / `Value2` array-shape assumptions, array lifecycle and dimension hazards, deterministic runtime failures proven by shared constant, type, and CFG/dataflow facts (`VBA249`), unsafe `Worksheet.Select` and `Range.Select` operations without proven active workbook or worksheet state (`VBA250`), typed Excel `Match`, `VLookup`, and `HLookup` calls that omit their match-mode argument (`VBA251`), unavailable typed `Excel.WorksheetFunction` members proved absent by the complete generated TypeLib member set (`VBA252`), repeated per-cell Excel work (`VBA225`), loop-invariant Excel member-chain resolution (`VBA238`), repeated `ReDim Preserve` copies in reachable loops (`VBA241`), failure information suppressed across local call chains, unsafe SQL construction (`VBA239`), recursive and cyclic procedure dependencies (`VBA244`, reported as one deterministic representative witness per cyclic SCC), and selected opt-in semantic risks. Opt-in `VBA240` adds project-wide module-state read/write metrics and structural lifecycle-coupling findings without enforcing fan-in thresholds. `VBA210` includes early exits, error-handler paths that return normally, and shared cleanup assignments; known non-returning `Err.Raise` statements are treated as exceptional exits rather than normal fallthrough. It does not treat VBA default initialization as an intentional return assignment and names a representative uncovered exit where practical. `VBA220` reports direct or uniquely resolved callee effects that can re-enter supported Excel or UserForm event handlers; ambiguous and unresolved calls remain uncertainty warnings. `VBA203` remains at the direct Application-state assignment, while batch-only `VBA221` reports the immediate call to a resolved local helper whose direct state change can leave the helper un-restored; it names the property, root cause, and any callee uncertainty without repeating the finding at ancestors. Batch-only `VBA222` checks public signatures against the project visibility and embedded/generated TypeLib database, conservatively warning on unresolved or ambiguous types while excluding host-required event handlers. `VBA227` uses a common CFG allocation state for fixed, dynamic, multidimensional, object, and Variant arrays; unique local array-return summaries can prove allocation, while ambiguous and external values remain unknown. `VBA229` reports unresolved type identifiers in procedure-local `Dim` and `Static ... As <Type>` declarations using the production VBA/host/TypeLib and project-symbol resolver. `VBA207`, `VBA213`, and `VBA230` through `VBA235` share Dictionary/Collection identity and helper-effect facts; the detailed safety and diagnostic-ownership contract is defined in [VBA Dictionary and Collection Safety Analysis](vba-dictionary-collection-safety.md). VBA237 reports the handler, cleanup boundary, resume-next scope, or ignored Boolean success result where a uniquely resolved local call chain loses failure information; uncertain calls do not prove a finding. Stable analyzer codes remain `VBA101` through `VBA106`, `VBA201` through `VBA258`, and `VBA260` through `VBA264`.

The dead-store addition extends the stable analyzer range through `VBA256`,
and the unused-declaration expansion extends it through `VBA264`: `VBA260`
reports unread private-procedure parameters, `VBA261` unreferenced module-level
`Private Const` declarations, `VBA262` unaccessed `Private Type` members,
`VBA263` scalar locals read but never assigned, and `VBA264` locals read on a
...
dispatched type information; `VBA260` through `VBA264` follow the same opt-in,
fail-open posture.

The opt-in `VBA242` rule reports expensive operations over full-row, full-column,
full-sheet, or unbounded `UsedRange` targets when configured.

VBA212 specifically analyzes AST operand relationships in eager `And`, `Or`,
`IIf`, `Choose`, and `Switch` expressions. It pairs `Nothing` and `IsArray`
guards with matching nested member, array/Collection/Dictionary index, and
`LBound`/`UBound` accesses; value-only or independently safe operands are
ignored. A property getter is included only when a unique same-project getter
has a proven direct or propagated effect (full project in batch, active
document in realtime); unresolved, external, ambiguous, and effect-free
