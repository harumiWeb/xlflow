...
| -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------- |
| `VBA260` | A parameter of a `Private` procedure that is never referenced in the procedure body is reported as a warning.                                                | Disabled; opt-in |
| `VBA261` | A module-level `Private Const` never referenced by any expression, declaration initializer, or conditional-compilation directive in the module is a warning. | Disabled; opt-in |
| `VBA262` | A member of a `Private Type` that no resolvable member expression in the module touches is an information finding.                                           | Disabled; opt-in |
| `VBA263` | A procedure-local scalar variable that is read but never assigned is a warning.                                                                              | Disabled; opt-in |
| `VBA264` | A procedure-local scalar variable read on a reachable path without a guaranteed assignment is a warning.                                                     | Disabled; opt-in |
...
file-local, so `VBA261`/`VBA262` need no project view and run in realtime.
`VBA260` restricts to `Private` procedures because `Public`/`Friend`
...
`VBA263`/`VBA264` share one candidate pass over `ProcedureIR` accesses scoped
...
`VBA262` resolves member usage through qualified and `With`-implicit member
...
- `VBA260`/`VBA261` use lexical whole-word scans that treat comments and
  ...
  only in prose.
- `VBA263`/`VBA264` cover only scalar locals; objects, Variants, arrays,
...
2. **Semantic use detection for `VBA260`/`VBA261` via `VariableAccess`
...
3. **Whole-project UDT member resolution.** Rejected because `Private Type`
   ...
   improving precision.
4. **Count `ByRef` arguments as reads for `VBA264`.** Rejected because
