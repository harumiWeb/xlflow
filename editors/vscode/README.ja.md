...
![ドキュメンテーション文字列のサポート](https://raw.githubusercontent.com/harumiWeb/xlflow/main/editors/vscode/images/docstrings.png)

<p align="center">
<small>ドキュメンテーション文字列の生成とホバー表示</small>
</p>
...
- 既存の `.xlsm` / `.xlam` / `.xlsb` マクロブックをxlflowプロジェクト化するには、拡張機能サイドバーから `既存ブックを xlflow プロジェクト化` をクリックし、マクロブックを選択します。
...
| `xlflow: Initialize Project`          | 既存ブックを xlflow プロジェクト化し、設定ファイルを作成します。        |
