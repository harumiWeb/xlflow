...
public sealed class EditCommand : ICommandHandler
{
    ...
    public BridgeResponse Handle(BridgeRequest request, CancellationToken cancellationToken)
    {
        ...
        var args = new EditCommandArguments(
            ...
            Name: BridgePayload.GetString(request.Payload, "Name") ?? "",
            Before: BridgePayload.GetString(request.Payload, "Before") ?? "",
            After: BridgePayload.GetString(request.Payload, "After") ?? "",
            IfMissing: BridgePayload.GetBool(request.Payload, "IfMissing"),
