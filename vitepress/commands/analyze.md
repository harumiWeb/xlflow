...
`VBA252` is enabled by default and reports typed
`Excel.WorksheetFunction` member calls that are absent from the complete
generated TypeLib member set. It uses the normal typed member resolver rather
than a hand-written worksheet-function allowlist. Incomplete TypeLib data and
unresolved, late-bound, user-defined, `Object`, or `Variant` receivers remain
silent. Use `detect_unavailable_worksheet_function_members`,
