...
`VBA250` is enabled by default and reports intentional Excel UI selections
whose active-state preconditions are not proven within the procedure. A
`Workbook.Activate` establishes the workbook, a `Worksheet.Activate`
establishes its parent workbook and worksheet, `Worksheet.Select` requires the
parent workbook, and `Range.Select` requires the owning worksheet. Conditional
activation, unknown calls, and helper-procedure effects remain conservative at
branch joins. Use `detect_unsafe_select_operations`,
`[analyze].disabled_rules = ["VBA250"]`, or an inline suppression for an
intentional exception. `VBA250` is independent of the `VB003` style warning
about avoidable `Select`/`Activate` usage.
...
`VBA251` is enabled by default and reports typed Excel `Match`, `VLookup`, and
`HLookup` calls that omit the optional match-mode argument. Explicit exact or
approximate values are accepted, and the rule does not assess whether an
explicit approximate lookup is correct for the data. Use
`detect_implicit_approximate_lookups`,
`[analyze].disabled_rules = ["VBA251"]`, or an inline suppression for an
