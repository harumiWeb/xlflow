...
| `source_encoding_invalid`                                      | A managed `.bas`, `.cls`, or `.frm` is not UTF-8 without BOM. | Run `xlflow encoding check`, then convert CP932 input with `xlflow encoding convert --from cp932`. |
