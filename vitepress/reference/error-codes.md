...
Lint codes include `VB001` through `VB015`, `VB018` through `VB023`, and `VB026` through `VB066`. `VB066` is a non-blocking style warning for VBE-accepted `Property Get` terminators; the other compile-dialog prevention range remains `VB065` and below. Analyzer codes include `VBA101` through `VBA106`, runtime-risk findings `VBA201` through `VBA259`, Excel semantic inspections `VBA260` through `VBA262`, unused-declaration findings `VBA265` through `VBA269`, the worksheet cell-reference collision finding `VBA270`, and `Option Base` inconsistency findings `VBA271` and `VBA272`; `VBA248` is a maintainability finding, `VBA250` is an active-state runtime-safety finding, `VBA251` is a reliability warning for implicit approximate lookup mode, `VBA252` is a correctness warning for unavailable typed `WorksheetFunction` members, `VBA253` through `VBA255` cover implicit, unbound, and bang default-member access, `VBA256` covers dead scalar stores, `VBA257` and `VBA258` cover discarded Function return values, `VBA259` covers unreachable `Select Case` branches, `VBA260` through `VBA262` cover worksheet-name access, worksheet-function dispatch, and host bracket expressions, `VBA265` through `VBA269` cover unused parameters, private constants, user-defined type members, and unassigned or prematurely read variables, `VBA270` covers worksheet-visible functions named after valid cell references, and `VBA271` and `VBA272` cover zero-based `ParamArray` parameters and qualified `VBA.Array` calls under `Option Base 1`.
...
deterministic failure. These findings are
non-blocking. `VBA202` remains the owner of proven error-91 object-use-before-
`Set`/`Nothing` failures.
