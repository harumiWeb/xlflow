...
| `detect_unused_parameters`                      | bool     | no       | `false` | Opt in to `VBA260` warnings for unused parameters of private procedures.                                                                                           |
| `detect_unused_private_constants`               | bool     | no       | `false` | Opt in to `VBA261` warnings for unreferenced module-level `Private Const` declarations.                                                                            |
| `detect_unused_udt_members`                     | bool     | no       | `false` | Opt in to `VBA262` diagnostics for members of `Private Type` declarations that are never accessed through a resolvable member expression.                          |
| `detect_never_assigned_variables`               | bool     | no       | `false` | Opt in to `VBA263` warnings for local scalar variables that are read but never assigned.                                                                           |
| `detect_unassigned_variable_usage`              | bool     | no       | `false` | Opt in to `VBA264` warnings for local scalar variables read before any guaranteed assignment.                                                                      |
