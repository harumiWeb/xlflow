...
| `detect_implicit_byref_parameters`              | bool     | no       | `false` | Opt in to `VBA270` information diagnostics for parameters that implicitly default to `ByRef`.                                                                      |
| `detect_assigned_byval_parameters`              | bool     | no       | `false` | Opt in to `VBA271` warnings for reassigned parameters with effective `ByVal` semantics.                                                                            |
| `detect_byref_parameters_can_be_byval`          | bool     | no       | `false` | Opt in to `VBA272` information diagnostics for `ByRef` parameters proven not to replace caller arguments.                                                          |
| `detect_misleading_property_value_byref`        | bool     | no       | `false` | Opt in to `VBA273` warnings for explicit `ByRef` on Property Let/Set final value parameters, which VBA passes `ByVal`.                                             |
| `detect_redundant_byref_modifiers`              | bool     | no       | `false` | Opt in to `VBA274` information diagnostics for explicit `ByRef` that repeats VBA's default. Cannot be enabled with `detect_implicit_byref_parameters`.             |
...
`detect_implicit_approximate_lookups = false` is the compatibility key for disabling `VBA251`; prefer `disabled_rules = ["VBA251"]` for policy suppression.

`detect_unavailable_worksheet_function_members = false` is the compatibility key for disabling `VBA252`; prefer `disabled_rules = ["VBA252"]` for policy suppression.

`detect_implicit_default_member_access = true` opts in to `VBA253`.
`detect_unbound_default_member_access = true` opts in to
`VBA254`, and `detect_bang_notation = true` opts in to `VBA255`; prefer
`disabled_rules` for explicit project policy.
