...
- Added native `.NET` bridge support for `xlflow macros --bridge dotnet --json` and `xlflow run Module1.Main --bridge dotnet --json`, enabling macro discovery and execution through the .NET Excel bridge without PowerShell. Supports typed arguments, fully qualified macro names, save/no-save/save-as, timeout, session attachment, and structured error handling for `macro_failed`, `macro_not_found`, and `macro_disabled`. Auto mode keeps the existing PowerShell behavior for macros/run; use `--bridge dotnet` explicitly to route through the .NET bridge.
