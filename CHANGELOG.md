...
- Added native `.NET` bridge support for `xlflow trace enable|status|disable|clean --bridge dotnet --json`, enabling trace helper module management through the .NET bridge. `enable` installs the `XlflowTrace` module and optionally writes `XlflowTrace.bas` to the modules directory. `disable` validates source-match safety before changing the workbook. `status` reports injection state. `clean` removes trace logs without opening Excel. Auto mode keeps the existing PowerShell behavior; use `--bridge dotnet` explicitly to route through the .NET bridge.
- Enhanced `xlflow run --bridge dotnet` with full trace support (`--trace`), MsgBox/InputBox/FileDialog response injection (`--msgbox`, `--inputbox`, `--filedialog`), UI stream pipe support (`--ui-stream`), and `__XLFLOW_DEBUG_PIPE__` injection for `XlflowDebug.Log` transport. Previously these options were rejected by the .NET bridge; they are now fully supported with the same behavior as the PowerShell bridge.
