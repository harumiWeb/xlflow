...
| `form snapshot` | Designer の UserForm state を JSON/YAML spec に保存  | `xlflow form snapshot UserForm1 --designer --out src/forms/UserForm1.form.yaml --json` |
...
workbook の `UserForm` について、現在の VBIDE Designer state をレビューしやすい JSON / YAML spec ファイルとして保存します。
...
この command は `xlflow inspect form --designer` と同じ Excel COM 経路を再利用し、その結果を Go 側で安定した spec に変換して保存します。snapshot 専用の PowerShell / VBIDE 読み取り経路は増やしません。

`--designer` と `--out` は必須です。出力拡張子と実際のシリアライズ形式は必ず一致し、`.json` は JSON、`.yaml` / `.yml` は YAML を書きます。それ以外の拡張子は Excel を開く前に失敗します。
